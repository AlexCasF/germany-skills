...
async function runCompactSearch(path, params, parsed) {
...
async function buildSource(argv) {
    const parsed = parseArgs(argv);
...
async function sourceText(source) {
    ...
    let chosen = links.find((link) => link.kind === "html")?.url;
    if (!chosen) {
        chosen = links.find((link) => link.kind === "xml")?.url;
    }
    if (!chosen) {
...
function fillPath(path, parsed, command) {
    const flags = parsed.flags;
...
function httpGetAbsolute(rawUrl) {
...
function recordPath(docType, docId) {
    if (docType === "case-law") {
...
function inferRecordIdentity(record) {
    if (record["@type"] === "SearchResult" && typeof record.item === "object") {
...
function addKeyValue(params, raw) {
...
function envelope(command, summary, pathOrUrl) {
