...
async function main(argv: string[]): Promise<number> {
  ...
  try {
    ...
    else throw new CLIError(2, "unknown_command", "unknown command; run deutschlandatlas --help");
  } catch (error) {
...
async function runTableSource(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const table = requiredTable(parsed);
  const [layer, layerSource] = flagBool(parsed, "skip-layer-discovery") ? [0, "raw_default"] as [number, string] : await resolveLayer(table, parsed);
...
async function resolveLayer(table: string, parsed: ParsedArgs): Promise<[number, string]> {
  if (flagBool(parsed, "raw-layer-zero")) return [0, "raw_layer_zero"];
