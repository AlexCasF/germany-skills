...
async function main(argv) {
    ...
    try {
        ...
        else if (matches(argv, "tables", "search"))
            await runTablesSearch(argv.slice(2));
        else if (matches(argv, "table", "query"))
            await runTableQuery(argv.slice(2));
        else if (matches(argv, "table", "fields"))
            await runTableFields(argv.slice(2));
        else if (matches(argv, "table", "sample"))
            await runTableSample(argv.slice(2));
        else if (matches(argv, "table", "source"))
            await runTableSource(argv.slice(2));
        else if (matches(argv, "indicator", "dossier"))
            await runIndicatorDossier(argv.slice(2));
...
async function runTablesSearch(argv) {
    ...
    if (!term)
        throw new CLIError(2, "missing_term", "tables search requires --term");
...
async function runTableQuery(argv) {
    const parsed = parseArgs(argv);
    ...
    if (!params.resultRecordCount && !params.resultrecordcount)
        params.resultRecordCount = String(limitFlag(parsed, DEFAULT_LIMIT, SAFE_LIMIT));
...
async function runTableFields(argv) {
    const parsed = parseArgs(argv);
    const table = requiredTable(parsed);
...
async function runTableSample(argv) {
    ...
    if (data.exceededTransferLimit)
        warnings.push("ArcGIS reported exceededTransferLimit=true; narrow the where clause or paginate deliberately.");
    if (flagBool(parsed, "geometry"))
        warnings.push("Geometry was requested intentionally; outputs can grow quickly.");
...
async function runIndicatorDossier(argv) {
    ...
    try {
        ...
        if (sample.exceededTransferLimit)
            warnings.push("Sample query reports exceededTransferLimit=true; use pagination/filtering for full extraction.");
    }
    catch (error) {
...
async function runQueryBuilder(argv) {
    ...
    if (parsed.flags.region && where === "1=1")
        where = `name LIKE '%${parsed.flags.region.replace(/'/g, "''")}%'`;
    const params = { f: "json", where, outFields: firstNonEmpty(parsed.flags.fields, "*"), returnGeometry: boolString(flagBool(parsed, "geometry")), resultRecordCount: String(limit) };
    Object.assign(params, parsed.params);
...
async function runExplainField(argv) {
    ...
    if (!fieldName)
        throw new CLIError(2, "missing_field", "explain-field requires --field");
    const [layer] = await resolveLayer(table, parsed);
    ...
    if (!match)
        throw new CLIError(2, "field_not_found", "field not found in layer metadata");
...
function parseArgs(args) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (key === "param") {
            ...
            if (split > 0)
                parsed.params[value.slice(0, split)] = value.slice(split + 1);
        }
        else {
...
function requiredTable(parsed) {
    const table = firstNonEmpty(parsed.flags.table, parsed.flags.name, firstPosition(parsed));
    if (!table)
        throw new CLIError(2, "missing_table", "command requires --table");
    return table;
}
async function resolveLayer(table, parsed) {
    ...
    if (layerFlag && layerFlag !== "auto") {
        ...
        if (!Number.isFinite(layer))
            throw new CLIError(2, "invalid_layer", "--layer must be auto or an integer");
        return [layer, "explicit_flag"];
    }
    const data = await fetchJson(serviceUrl(table));
    for (const layer of asArray(data.layers)) {
        if (String(layer.type ?? "").toLowerCase().includes("feature"))
            return [intValue(layer.id), "service_metadata"];
    }
    if (asArray(data.layers).length > 0)
        return [intValue(asArray(data.layers)[0].id), "service_metadata"];
...
function portalSearchUrl(term, limit, start) {
...
function compactFields(layerData) {
    return asArray(layerData.fields).map((field) => ({ name: field.name, alias: field.alias, type: field.type, length: intValue(field.length), domain: field.domain }));
}
function compactFeatures(data, includeGeometry) {
    return asArray(data.features).map((feature) => {
        const item = { attributes: feature.attributes };
        if (includeGeometry)
            item.geometry = feature.geometry;
        return item;
    });
}
function likelyIndicatorFields(fields) {
...
function firstLikelyIndicator(fields) {
...
function serviceSummary(data) {
    return {
        ...
        layers: asArray(data.layers).map((layer) => ({ id: intValue(layer.id), name: layer.name, type: layer.type }))
    };
}
function sourcesForTable(table, layer) {
    return [
...
function limitFlag(parsed, fallback, max) {
    ...
    if (value > max && !flagBool(parsed, "allow-large-output"))
        throw new CLIError(2, "limit_exceeds_safe_max", `limit ${value} exceeds safe max ${max}; pass --allow-large-output to override`);
    return value;
}
...
function tableFromUrl(raw) {
    const parts = raw.replace(/^\/+|\/+$/g, "").split("/");
    for (let i = 0; i < parts.length; i++) {
        if (parts[i] === "services" && i + 1 < parts.length)
            return parts[i + 1];
    }
    return "";
}
