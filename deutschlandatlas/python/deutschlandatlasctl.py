...
def main(argv):
    ...
    try:
        if argv[0] == "doctor":
            ...
            raise CLIError(2, "unknown_command", "unknown command; run deutschlandatlasctl --help")
...
def print_root_help():
    print("""deutschlandatlasctl -- Deutschlandatlas ArcGIS research CLI
...
def print_help(path):
    ...
    if joined == "table sample":
        print("""deutschlandatlasctl table sample
...
def run_table_fields(argv):
    ...
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"deutschlandatlasctl table sample --table {table} --fields name,{first_likely_indicator(fields)} --limit 5", f"deutschlandatlasctl indicator dossier --table {table}"]
    emit(payload)
...
def run_table_sample(argv):
    ...
    payload["sources"] = sources_for_table(table, layer)
    payload["warnings"] = warnings
    payload["nextActions"] = [f"deutschlandatlasctl table fields --table {table}", f"deutschlandatlasctl query-builder --table {table} --where \"name LIKE '%Berlin%'\" --fields name,* --limit 10"]
...
def run_table_source(argv):
    ...
    payload["sources"] = sources_for_table(table, layer)
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"deutschlandatlasctl table fields --table {table}", f"deutschlandatlasctl table sample --table {table} --limit 5"]
    emit(payload)
...
def run_indicator_dossier(argv):
    ...
    payload["sources"] = sources_for_table(table, layer)
    payload["warnings"] = warnings
    payload["nextActions"] = [f"deutschlandatlasctl table fields --table {table}", f"deutschlandatlasctl table sample --table {table} --fields name,* --where \"1=1\" --limit 10", f"deutschlandatlasctl explain-field --table {table} --field {first_likely_indicator(fields)}"]
    emit(payload)
...
def run_query_builder(argv):
    ...
    payload["nextActions"] = [f"deutschlandatlasctl table query --table {table} --layer {layer} --param where={where!r} --param outFields={params['outFields']!r} --limit {limit}"]
    emit(payload)
...
def run_explain_field(argv):
    ...
    payload["sources"] = sources_for_table(table, layer)
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"deutschlandatlasctl table sample --table {table} --fields name,{field_name} --limit 10"]
    emit(payload)
...
def compact_portal_results(data, limit):
    ...
    for item in (data.get("results") or [])[:limit]:
        ...
        items.append({
            ...
            "nextActions": [f"deutschlandatlasctl table fields --table {table}", f"deutschlandatlasctl indicator dossier --table {table}"],
        })
    return items
...
def next_actions_for_tables(items):
    return [f"deutschlandatlasctl indicator dossier --table {item['table']}" for item in items[:3] if item.get("table")] or ['deutschlandatlasctl tables search --term "Apotheken" --limit 5']
