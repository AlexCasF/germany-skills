...
func main() {
	...
	default:
		err = cliError{2, "unknown_command", "unknown command; run deutschlandatlasctl --help"}
	}
	if err != nil {
		var ce cliError
		if errors.As(err, &ce) {
...
func printRootHelp() {
	fmt.Println(`deutschlandatlasctl -- Deutschlandatlas ArcGIS research CLI
...
func printHelp(path []string) {
	...
	case "table sample":
		fmt.Println(`deutschlandatlasctl table sample
...
Examples
  ...
  deutschlandatlasctl table sample --table alq_HA2023 --geometry true --limit 2
...
Notes
  ...
  when you do not know the indicator field name.`)
	case "indicator dossier":
		fmt.Println(`deutschlandatlasctl indicator dossier
...
Example
  deutschlandatlasctl indicator dossier --table alq_HA2023 --limit 3`)
	case "tables search":
		fmt.Println(`deutschlandatlasctl tables search

Search the public ArcGIS portal for Deutschlandatlas table services.

Examples
  deutschlandatlasctl tables search --term "Apotheken" --limit 5
  deutschlandatlasctl tables search --term "Arbeitslosenquote" --limit 5`)
	default:
		printRootHelp()
	}
...
func runTableFields(argv []string) error {
	...
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{
		fmt.Sprintf("deutschlandatlasctl table sample --table %s --fields name,%s --limit 5", table, firstLikelyIndicator(fields)),
		fmt.Sprintf("deutschlandatlasctl indicator dossier --table %s", table),
	}
	emit(payload)
	return nil
}
...
func runTableSample(argv []string) error {
	...
	payload["nextActions"] = []string{
		fmt.Sprintf("deutschlandatlasctl table fields --table %s", table),
		fmt.Sprintf("deutschlandatlasctl query-builder --table %s --where \"name LIKE '%%Berlin%%'\" --fields name,* --limit 10", table),
	}
...
func runTableSource(argv []string) error {
	...
	payload["nextActions"] = []string{
		fmt.Sprintf("deutschlandatlasctl table fields --table %s", table),
		fmt.Sprintf("deutschlandatlasctl table sample --table %s --limit 5", table),
	}
	emit(payload)
	return nil
}
...
func runIndicatorDossier(argv []string) error {
	...
	payload["nextActions"] = []string{
		fmt.Sprintf("deutschlandatlasctl table fields --table %s", table),
		fmt.Sprintf("deutschlandatlasctl table sample --table %s --fields name,* --where \"1=1\" --limit 10", table),
		fmt.Sprintf("deutschlandatlasctl explain-field --table %s --field %s", table, firstLikelyIndicator(asSlice(payload["fields"]))),
	}
	emit(payload)
	return nil
}
...
func runQueryBuilder(argv []string) error {
	...
	payload["nextActions"] = []string{
		fmt.Sprintf("deutschlandatlasctl table query --table %s --layer %d --param where=%q --param outFields=%q --limit %d", table, layer, where, params.Get("outFields"), limit),
	}
	emit(payload)
	return nil
}
...
func runExplainField(argv []string) error {
	...
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{
		fmt.Sprintf("deutschlandatlasctl table sample --table %s --fields name,%s --limit 10", table, fieldName),
	}
	emit(payload)
	return nil
}
...
func nextActionsForTables(items []any) []string {
	actions := []string{}
	if len(items) == 0 {
		return []string{`deutschlandatlasctl tables search --term "Apotheken" --limit 5`}
	}
	for i, itemAny := range items {
		...
		if table != "" {
			actions = append(actions, fmt.Sprintf("deutschlandatlasctl indicator dossier --table %s", table))
		}
	}
	return actions
}
