...
func main() {
	...
	case args[0] == "geo":
		err = runGeo(args[1:])
	default:
		err = cliError{2, "unknown_command", "unknown command; run dashboard-deutschland --help"}
	}
	if err != nil {
		var ce cliError
		if errors.As(err, &ce) {
...
func printRootHelp() {
	fmt.Println(`dashboard-deutschland -- Dashboard Deutschland research CLI
...
Do not use this when
  - you need deep configurable statistical tables; use destatis
  - you need regional atlas tables; use deutschlandatlas or regionalatlas
...
func printHelp(path []string) {
	...
	case "indicator data":
		fmt.Println(`dashboard-deutschland indicator data
...
Flags
  ...
  --include-raw            Include parsed tile config`)
	case "dashboard dossier":
		fmt.Println(`dashboard-deutschland dashboard dossier

Bundle dashboard metadata and a small set of normalized indicator summaries.

Examples
  dashboard-deutschland dashboard dossier --id arbeitsmarkt --indicator-limit 3
  dashboard-deutschland dashboard dossier --name Arbeitsmarkt --indicator-limit 3`)
	case "geo":
...
func runDashboardsList(argv []string) error {
	...
	payload["nextActions"] = []string{"dashboard-deutschland dashboard dossier --id arbeitsmarkt --indicator-limit 3"}
	emit(payload)
	return nil
}
...
func runIndicatorGet(argv []string) error {
	...
	payload["warnings"] = warnings
	payload["nextActions"] = []string{
		fmt.Sprintf("dashboard-deutschland indicator data --id %s --limit 10", id),
		fmt.Sprintf("dashboard-deutschland indicator source --id %s", id),
	}
...
func runIndicatorData(argv []string) error {
	...
	payload["sources"] = sourcesForIndicator(indicator, config)
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{fmt.Sprintf("dashboard-deutschland indicator source --id %s", id)}
	if flagBool(parsed, "include-raw") {
		payload["raw"] = config
	}
	emit(payload)
...
func runIndicatorSource(argv []string) error {
	...
	payload["sources"] = sourcesForIndicator(indicator, config)
	payload["warnings"] = warnings
	payload["nextActions"] = []string{fmt.Sprintf("dashboard-deutschland indicator data --id %s --limit 10", id)}
	emit(payload)
	return nil
}
...
func runDashboardDossier(argv []string) error {
	...
	for _, id := range ids[:minInt(3, len(ids))] {
		payload["nextActions"] = append(payload["nextActions"].([]string), fmt.Sprintf("dashboard-deutschland indicator data --id %s --limit 10", id))
	}
	emit(payload)
	return nil
}
...
func compactDashboard(dashboard map[string]any) map[string]any {
	...
	return map[string]any{
		...
		"nextActions": []string{
			fmt.Sprintf("dashboard-deutschland dashboard dossier --id %s --indicator-limit 3", stringAt(dashboard, "id")),
		},
	}
}
...
func compactIndicators(indicators []map[string]any, limit int) []map[string]any {
	...
	for _, indicator := range indicators[:minInt(limit, len(indicators))] {
		...
		summary["nextActions"] = []string{
			fmt.Sprintf("dashboard-deutschland indicator data --id %s --limit 10", stringAt(indicator, "id")),
			fmt.Sprintf("dashboard-deutschland indicator source --id %s", stringAt(indicator, "id")),
		}
...
func nextActionsForIndicators(items []map[string]any) []string {
	...
	for _, item := range items[:minInt(3, len(items))] {
		id := stringAt(item, "id")
		actions = append(actions, fmt.Sprintf("dashboard-deutschland indicator get --id %s", id))
		actions = append(actions, fmt.Sprintf("dashboard-deutschland indicator data --id %s --limit 10", id))
	}
	if len(actions) == 0 {
		return []string{`dashboard-deutschland indicator search --term "Arbeitsmarkt" --limit 5`}
	}
	return actions
}
