...
OPENAPI_REPO_URL = "https://github.com/AndreasFischer1985/dashboard-deutschland-api"
DEFAULT_LIMIT = 10
SAFE_LIMIT = 100
...
def main(argv):
    ...
    try:
        if argv[0] == "doctor":
            ...
            raise CLIError(2, "unknown_command", "unknown command; run dashboardctl --help")
...
def print_root_help():
    print("""dashboardctl -- Dashboard Deutschland research CLI
...
def print_help(path):
    ...
    if joined == "indicator data":
        print("""dashboardctl indicator data --id <indicator-id> [--limit n]
...
""")
        ...
        print("""dashboardctl dashboard dossier --id <dashboard-id> [--indicator-limit n]

Bundle dashboard metadata and a small set of normalized indicator summaries.
""")
    elif joined == "geo":
        print("""dashboardctl geo
...
def run_indicator_get(argv):
    ...
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"dashboardctl indicator data --id {item_id} --limit 10", f"dashboardctl indicator source --id {item_id}"]
...
def run_indicator_data(argv):
    ...
    payload["sources"] = sources_for_indicator(indicator, config)
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"dashboardctl indicator source --id {item_id}"]
    if flag_bool(parsed, "include-raw"):
        payload["raw"] = config
    emit(payload)
...
def run_indicator_source(argv):
    ...
    payload["sources"] = sources_for_indicator(indicator, config)
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"dashboardctl indicator data --id {item_id} --limit 10"]
    emit(payload)
...
def run_dashboard_dossier(argv):
    ...
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"dashboardctl indicator data --id {item_id} --limit 10" for item_id in ids[:3]]
    emit(payload)
...
def compact_indicators(indicators, limit):
    ...
    for indicator in indicators[:limit]:
        ...
        summary["nextActions"] = [f"dashboardctl indicator data --id {indicator.get('id')} --limit 10", f"dashboardctl indicator source --id {indicator.get('id')}"]
        out.append(summary)
    return out
...
def next_actions_for_indicators(items):
    ...
    for item in items[:3]:
        item_id = item.get("id")
        actions.extend([f"dashboardctl indicator get --id {item_id}", f"dashboardctl indicator data --id {item_id} --limit 10"])
    return actions or ['dashboardctl indicator search --term "Arbeitsmarkt" --limit 5']
