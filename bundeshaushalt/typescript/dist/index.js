...
async function main(argv) {
    try {
        ...
        else if (match(argv, "years", "list"))
            runYearsList(argv.slice(2));
        else if (match(argv, "budget", "tree"))
            await runBudgetTree(argv.slice(2));
        else if (match(argv, "budget", "sample"))
            await runSample(argv.slice(2));
        else if (argv[0] === "sample")
            await runSample(argv.slice(1));
        else if (match(argv, "title", "get"))
            await runTitleGet(argv.slice(2));
        else if (argv[0] === "search")
            await runSearch(argv.slice(1));
        else if (argv[0] === "compare")
            await runCompare(argv.slice(1));
        else if (argv[0] === "budget-data")
            await runBudgetData(argv.slice(1));
        else
...
function printHelp(args) {
    if (args.length === 0)
        printRootHelp();
    else if (args[0] === "budget-data")
        console.log("budget-data flags: --year --account expenses|income --quota target|actual --unit single|function|group --id --param key=value --raw");
    else if (args[0] === "search")
        console.log("search flags: --year --account --quota --unit --term/--q --depth --max-requests --limit --include-raw");
    else if (match(args, "budget", "tree"))
        console.log("budget tree flags: --year --account --quota --unit --id --limit --grep --include-raw --raw");
    else if (args[0] === "compare")
        console.log("compare flags: --years 2024,2025 --account --quota --unit --id");
    else
        printRootHelp();
}
...
async function runTitleGet(argv) {
    const parsed = parseArgs(argv);
    if (!first(parsed.flags.id, parsed.params.id))
        throw new CliError("missing_id", "title get requires --id", 2);
    const params = budgetParams(parsed, true);
    const { raw, data, requestUrl } = await fetchBudget(params);
    if (flagBool(parsed, "raw")) {
        process.stdout.write(raw);
...
async function runSearch(argv) {
    ...
    if (!term)
        throw new CliError("missing_term", "search requires --term", 2);
    const params = budgetParams(parsed, true);
    delete params.id;
...
async function runCompare(argv) {
    ...
    if (!rawYears)
        throw new CliError("missing_years", "compare requires --years 2024,2025", 2);
    const years = rawYears.split(/[;,]/).map((part) => Number(part.trim())).filter((year) => Number.isFinite(year));
    if (years.length < 2)
        throw new CliError("missing_years", "compare needs at least two years", 2);
    ...
    for (const year of years) {
        ...
        try {
            const { data, requestUrl } = await fetchBudget(params);
            items.push({ year, ok: true, requestUrl, meta: data.meta, detail: compactElement(data.detail ?? {}, data.meta ?? {}, params), childCount: Array.isArray(data.children) ? data.children.length : 0 });
        }
        catch (error) {
...
async function runBudgetData(argv) {
    ...
    if (!params.year || !params.account)
        throw new CliError("missing_required_params", "budget-data requires --year and --account (or --param year=... --param account=...)", 2);
    const { raw, data, requestUrl } = await fetchBudget(params);
    if (flagBool(parsed, "raw")) {
        process.stdout.write(raw);
        return;
...
function emitBudgetEnvelope(command, requestUrl, params, data, parsed) {
    const limit = limitFlag(parsed, DEFAULT_LIMIT, SAFE_LIMIT);
    const grep = (parsed.flags.grep ?? "").toLowerCase();
    const children = Array.isArray(data.children) ? data.children : [];
    let items = children.map((child) => compactElement(child, data.meta ?? {}, params));
    if (grep)
        items = items.filter((item) => String(item.label ?? "").toLowerCase().includes(grep));
    if (limit >= 0)
        items = items.slice(0, limit);
    const payload = envelope(command, requestUrl, { ...params, limit, grep });
    payload.summary = {
        meta: data.meta,
        detail: compactElement(data.detail ?? {}, data.meta ?? {}, params),
        childrenTotal: children.length,
        childrenShown: items.length,
        parentsLevels: Array.isArray(data.parents) ? data.parents.length : 0,
        relatedKeys: data.related && typeof data.related === "object" ? Object.keys(data.related).sort() : null,
    };
    payload.items = items;
...
async function searchHierarchy(params, term, depthLimit, maxRequests, limit, includeRaw) {
    ...
    while (queue.length > 0 && requests < maxRequests && items.length < limit) {
        ...
        if (!node || seen.has(node.id) || node.depth > depthLimit)
            continue;
        seen.add(node.id);
        const requestParams = { ...params };
        if (node.id)
            requestParams.id = node.id;
        let data;
        let requestUrl;
        try {
        ...
        if (matchesElement(detail, needle)) {
            const item = compactElement(detail, data.meta ?? {}, requestParams);
        ...
        for (const child of Array.isArray(data.children) ? data.children : []) {
            ...
            if (matchesElement(child, needle)) {
                const item = compactElement(child, data.meta ?? {}, params);
            ...
            if (childId && node.depth + 1 <= depthLimit)
                queue.push({ id: childId, depth: node.depth + 1 });
        }
    }
    return { items, requests };
}
function compactElement(element, meta, params) {
...
async function fetchBudget(params) {
    const requestUrl = withParams(BUDGET_DATA_URL, params);
    const response = await fetchRaw(requestUrl);
    if (response.status < 200 || response.status >= 300)
        throw new CliError("upstream_http_error", `upstream status ${response.status} from ${requestUrl}: ${stripSpace(response.body).slice(0, 300)}`);
    return { raw: response.body, data: JSON.parse(response.body), requestUrl };
}
...
function budgetParams(parsed, requireYear, requireAccount = false) {
    ...
    params.quota ??= "target";
    params.unit ??= "single";
    if (requireYear && !params.year)
        throw new CliError("missing_year", "command requires --year", 2);
    if (requireAccount && !params.account)
        throw new CliError("missing_account", "command requires --account", 2);
...
function validateBudgetParams(params) {
    if (params.year) {
        const year = Number(params.year);
        if (!Number.isInteger(year) || year < 2000 || year > 2100)
            throw new CliError("invalid_year", "year must be a plausible four-digit year", 2);
    }
    if (params.account && !["expenses", "income"].includes(params.account))
        throw new CliError("invalid_account", "account must be expenses or income", 2);
    if (params.quota && !["target", "actual"].includes(params.quota))
        throw new CliError("invalid_quota", "quota must be target or actual", 2);
    if (params.unit && !["single", "function", "group"].includes(params.unit))
        throw new CliError("invalid_unit", "unit must be single, function, or group", 2);
}
...
function defaultWarningsForResponse(data) {
    const warnings = defaultWarnings();
    const meta = (data.meta ?? {});
    if (meta.quota === "actual" && Number(meta.year ?? 0) > LATEST_ACTUAL_YEAR)
        warnings.push("This actual/Ist year is newer than the latest actual year observed during testing; verify availability carefully.");
    if (meta.unit === "function" || meta.unit === "group")
        warnings.push("Function and group views classify titles differently from Einzelplan ministry structure; do not mix categories without saying so.");
    return warnings;
}
...
function intFlag(parsed, key, fallback, max) {
    if (!parsed.flags[key])
        return fallback;
    const number = Number(parsed.flags[key]);
    if (!Number.isInteger(number))
        throw new CliError("invalid_integer", `--${key} must be an integer`, 2);
    return Math.max(0, Math.min(number, max));
}
function limitFlag(parsed, fallback, max) {
    return intFlag(parsed, "limit", fallback, max);
}
function matchesElement(element, needle) {
