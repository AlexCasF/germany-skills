...
function nextActionsFromChildren(items: JsonObject[]): string[] {
  ...
  for (const item of items.slice(0, 3)) {
    if (item.id) actions.push(`bundeshaushaltctl budget tree --year ${item.year} --account ${item.account} --quota ${item.quota} --unit ${item.unit} --id ${item.id} --limit 10`);
  }
  return actions;
}
...
function nextActionsFromSearch(items: JsonObject[], params: Record<string, string>): string[] {
  ...
  if (actions.length === 0) actions.push(`bundeshaushaltctl budget tree --year ${params.year} --account ${params.account} --limit 8`);
  return actions;
}
