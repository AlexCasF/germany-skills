...
func main() {
	...
	default:
		err = cliError{2, "unknown_command", "unknown command; run bundeshaushaltctl --help"}
	}
	if err != nil {
		var ce cliError
		if errors.As(err, &ce) {
...
func printRootHelp() {
	fmt.Println(`bundeshaushaltctl -- Bundeshaushalt Digital research CLI
...
func printHelp(path []string) {
	...
	case "budget tree":
		fmt.Println(`bundeshaushaltctl budget tree
...
Flags
  ...
  --raw               Print upstream JSON`)
	case "search":
		fmt.Println(`bundeshaushaltctl search
...
Flags
  ...
  --limit <n>            Result cap, defaults to 10`)
	case "title get":
		fmt.Println(`bundeshaushaltctl title get

Fetch one precise budget node by ID.

Examples
  bundeshaushaltctl title get --year 2025 --account expenses --id 110168112
  bundeshaushaltctl title get --year 2025 --account expenses --id 1101 --limit 8`)
	case "compare":
		fmt.Println(`bundeshaushaltctl compare

Compare one node across years.

Examples
  bundeshaushaltctl compare --years 2024,2025 --account expenses --id 110168112
  bundeshaushaltctl compare --years 2024,2025,2026 --account expenses`)
	default:
		printRootHelp()
	}
}
...
9. Query by function instead of Einzelplan:
   bundeshaushaltctl budget tree --year 2025 --account expenses --unit function --limit 8
...
func runFields(argv []string) error {
	...
	payload["sources"] = defaultSources()
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{"bundeshaushaltctl budget tree --year 2026 --account expenses --limit 8"}
	emit(payload)
	return nil
}
...
func runSource(argv []string) error {
	...
	payload["sources"] = defaultSources()
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{"bundeshaushaltctl fields", "bundeshaushaltctl years list"}
	emit(payload)
	return nil
}
...
func runYearsList(argv []string) error {
	...
	for _, year := range knownYears {
		items = append(items, map[string]any{
			...
			"exampleTargetCmd": fmt.Sprintf("bundeshaushaltctl budget tree --year %d --account expenses --quota target --limit 8", year),
		})
	}
	payload["summary"] = map[string]any{
	...
	payload["sources"] = defaultSources()
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{"bundeshaushaltctl budget tree --year 2026 --account expenses --quota target --limit 8"}
	emit(payload)
	return nil
}
...
func runCompare(argv []string) error {
	...
	payload["sources"] = defaultSources()
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{"bundeshaushaltctl source"}
	emit(payload)
	return nil
}
...
func nextActionsForElement(id string, params url.Values) []string {
	...
	return []string{
		fmt.Sprintf("bundeshaushaltctl title get --year %s --account %s --quota %s --unit %s --id %s", year, account, quota, unit, id),
		fmt.Sprintf("bundeshaushaltctl budget tree --year %s --account %s --quota %s --unit %s --id %s --limit 10", year, account, quota, unit, id),
	}
}

func nextActionsForResponse(resp *budgetResponse, params url.Values) []string {
	actions := []string{"bundeshaushaltctl source"}
	for _, child := range resp.Children {
		if child.ID != "" {
			actions = append(actions, fmt.Sprintf("bundeshaushaltctl budget tree --year %s --account %s --quota %s --unit %s --id %s --limit 10", params.Get("year"), params.Get("account"), params.Get("quota"), params.Get("unit"), child.ID))
		}
		if len(actions) >= 4 {
			break
		}
...
func nextActionsFromSearch(items []map[string]any, params url.Values) []string {
	...
	for _, item := range items {
		if id := fmt.Sprint(item["id"]); id != "" {
			actions = append(actions, fmt.Sprintf("bundeshaushaltctl title get --year %s --account %s --quota %s --unit %s --id %s", params.Get("year"), params.Get("account"), params.Get("quota"), params.Get("unit"), id))
		}
		if len(actions) >= 4 {
			return actions
		}
	}
	return []string{"bundeshaushaltctl budget tree --year " + params.Get("year") + " --account " + params.Get("account") + " --limit 8"}
}
