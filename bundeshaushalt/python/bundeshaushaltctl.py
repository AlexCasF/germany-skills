...
def print_root_help() -> None:
    print(
        """bundeshaushaltctl 2.0 - Bundeshaushalt Digital research CLI
...
def run_doctor(argv: list[str]) -> None:
    ...
    payload["nextActions"] = [
        "bundeshaushaltctl years list",
        f"bundeshaushaltctl budget tree --year {LATEST_TARGET_YEAR} --account expenses --quota target --limit 8",
        'bundeshaushaltctl search --year 2025 --account expenses --term "Arbeit" --limit 5',
    ]
    emit(payload)
...
def run_fields(argv: list[str]) -> None:
    ...
    payload["sources"] = default_sources()
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"bundeshaushaltctl budget tree --year {LATEST_TARGET_YEAR} --account expenses --limit 8"]
    emit(payload)
...
def run_years_list(argv: list[str]) -> None:
    items = [
        {
            ...
            "exampleTargetCmd": f"bundeshaushaltctl budget tree --year {year} --account expenses --quota target --limit 8",
        }
    ...
    payload["sources"] = default_sources()
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"bundeshaushaltctl budget tree --year {LATEST_TARGET_YEAR} --account expenses --quota target --limit 8"]
    emit(payload)
...
def compact_element(element: dict[str, Any], meta: dict[str, Any], params: dict[str, str]) -> dict[str, Any]:
    ...
    if item["id"]:
        item["nextActions"] = [
            f"bundeshaushaltctl title get --year {item['year']} --account {item['account']} --quota {item['quota']} --unit {item['unit']} --id {item['id']}",
            f"bundeshaushaltctl budget tree --year {item['year']} --account {item['account']} --quota {item['quota']} --unit {item['unit']} --id {item['id']} --limit 10",
        ]
    return item
...
def next_actions_from_children(items: list[dict[str, Any]]) -> list[str]:
    actions = ["bundeshaushaltctl source"]
    for item in items[:3]:
        if item.get("id"):
            actions.append(f"bundeshaushaltctl budget tree --year {item.get('year')} --account {item.get('account')} --quota {item.get('quota')} --unit {item.get('unit')} --id {item.get('id')} --limit 10")
    return actions
...
def next_actions_from_search(items: list[dict[str, Any]], params: dict[str, str]) -> list[str]:
    ...
    for item in items[:3]:
        if item.get("id"):
            actions.append(f"bundeshaushaltctl title get --year {item.get('year')} --account {item.get('account')} --quota {item.get('quota')} --unit {item.get('unit')} --id {item.get('id')}")
    if not actions:
        actions.append(f"bundeshaushaltctl budget tree --year {params.get('year')} --account {params.get('account')} --limit 8")
    return actions
