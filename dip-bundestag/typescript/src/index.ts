...
function printHelpFor(path: string[]): void {
...
async function runPersonSearch(args: string[]): Promise<void> {
  const { flags } = parseArgs(args);
  const name = flags.name;
  if (!name) throw new CliError(2, "invalid_arguments", "missing required flag --name");
...
async function runPersonDossier(args: string[]): Promise<void> {
  const { flags } = parseArgs(args);
  const key = mustKey(flags);
  const limit = positiveInt(flags.limit ?? "10", "limit");
  let id = flags.id;
  ...
  if (!id) {
    if (!flags.name) throw new CliError(2, "invalid_arguments", "pass --id or --name");
    const { body, requestUrl } = await apiGet("person", { "f.person": [flags.name], format: ["json"] }, key);
    const docs = takeDocuments(JSON.parse(body), 1);
    if (docs.length === 0) throw new CliError(1, "not_found", "no person found for --name");
    id = String(docs[0].id);
  ...
  try {
    ...
    if (err instanceof CliError) warnings.push("Related activities could not be loaded: " + err.message);
  }
  writeJson({
    status: "ok",
    tool: APP_NAME,
...
async function runVorgangDossier(args: string[]): Promise<void> {
  const { flags } = parseArgs(args);
  const id = flags.id;
  if (!id) throw new CliError(2, "invalid_arguments", "missing required flag --id");
  const key = mustKey(flags);
  ...
  try {
    ...
    if (err instanceof CliError) warnings.push("Related positions could not be loaded: " + err.message);
  }
  writeJson({
    status: "ok",
    tool: APP_NAME,
...
async function runSource(args: string[]): Promise<void> {
  const { flags } = parseArgs(args);
  const entity = flags.type ?? flags.entity;
  if (!entity) throw new CliError(2, "invalid_arguments", "missing required flag --type");
  if (!ENTITIES.has(entity)) throw new CliError(2, "invalid_arguments", "unknown --type: " + entity);
  const key = mustKey(flags);
  ...
  if (id) sources.push({ title: "DIP API " + entity + " detail", url: BASE_URL + "/" + entity + "/" + id, kind: "api" });
  writeJson({
    status: "ok",
    tool: APP_NAME,
    command: "source",
...
async function runDocumentText(kind: string, args: string[]): Promise<void> {
  const { flags } = parseArgs(args);
  const key = mustKey(flags);
...
async function runPlenarySpeechSearch(args: string[]): Promise<void> {
  const { flags } = parseArgs(args);
  const term = flags.term;
  if (!term) throw new CliError(2, "invalid_arguments", "missing required flag --term");
  ...
  const key = mustKey(flags);
  const limit = positiveInt(flags.limit ?? "10", "limit");
  const params: Params = { format: ["json"] };
  if (flags["person-id"]) params["f.person_id"] = [flags["person-id"]];
  else if (flags.person) params["f.person"] = [flags.person];
  else throw new CliError(2, "invalid_arguments", "pass --document-number, --person-id, or --person");
...
async function resolveRecord(entity: string, id: string | undefined, documentNumber: string | undefined, key: string): Promise<{ record: JsonObject; request: JsonObject }> {
  if (id) {
  ...
  if (!documentNumber) throw new CliError(2, "invalid_arguments", "pass --id or --document-number");
  ...
  if (docs.length === 0) throw new CliError(1, "not_found", "no record found for document number");
  return { record: docs[0], request: requestMeta(resp.requestUrl) };
}

async function apiGet(path: string, params: Params, key: string): Promise<{ body: string; requestUrl: string }> {
  const u = new URL(BASE_URL + "/" + path.replace(/^\/+/, ""));
  for (const [paramKey, values] of Object.entries(params)) {
    for (const value of values) u.searchParams.append(paramKey, value);
  }
...
function httpsGet(requestUrl: string, headers: Record<string, string>): Promise<{ statusCode: number; body: string }> {
...
function parseArgs(args: string[]): { flags: Flags; params: Params } {
  const flags: Flags = {};
  const params: Params = {};
  for (let i = 0; i < args.length; i += 1) {
    const arg = args[i];
    if (!arg.startsWith("--")) throw new CliError(2, "invalid_arguments", "unexpected positional argument: " + arg);
    ...
    if (name === "param") {
      ...
      if (cut < 1) throw new CliError(2, "invalid_arguments", "--param must be key=value");
...
function resolveKey(flags: Flags): [string, string] {
  if (flags.apikey) return [flags.apikey, "flag"];
  if (process.env.DIP_API_KEY) return [process.env.DIP_API_KEY, "env:DIP_API_KEY"];
  return ["", "missing"];
}

function mustKey(flags: Flags): string {
  const [key] = resolveKey(flags);
  if (!key) throw new CliError(2, "missing_api_key", "set DIP_API_KEY or pass --apikey");
  return key;
}

function compactItem(doc: JsonObject): JsonObject {
  ...
  if (doc.titel) out.title = doc.titel;
  const sources = extractSources(doc);
  if (sources.length > 0) out.sources = sources;
  return out;
}

function extractSources(value: any): JsonObject[] {
  const out: JsonObject[] = [];
  const walk = (node: any, key = ""): void => {
    if (Array.isArray(node)) {
      ...
      else if (lower.includes("xml")) kind = "xml";
      else if (lower.includes("api")) kind = "api";
...
function dedupeSources(sources: JsonObject[]): JsonObject[] {
...
function snippets(text: string, term: string, context: number): JsonObject[] {
  const out: JsonObject[] = [];
...
function takeDocuments(data: JsonObject, limit: number): JsonObject[] {
  if (!Array.isArray(data.documents)) return [];
  return data.documents.slice(0, limit).filter((item: any) => item && typeof item === "object");
}

function requestMeta(requestUrl: string): JsonObject {
  return { method: "GET", url: redactUrl(requestUrl), redactions: ["Authorization", "apikey"] };
}

function redactUrl(requestUrl: string): string {
...
function positiveInt(raw: string, name: string): number {
  ...
  if (!Number.isFinite(value) || value < 1) throw new CliError(2, "invalid_arguments", `--${name} must be a positive integer`);
  return value;
}

function docSources(): JsonObject[] {
  return [
