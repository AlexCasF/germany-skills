...
func printRootHelp() {
	fmt.Printf(`dipctl -- official Bundestag DIP research CLI
...
Fast paths
  ...
  Get official plenary text snippets:
    dipctl plenarprotokoll text --document-number "20/139" --grep "Buergergeld"
...
Additional endpoint commands
  dipctl vorgangsposition list|get
  dipctl drucksache-text list|get
  dipctl plenarprotokoll-text list|get
...
Examples
  dipctl doctor
  dipctl doctor --apikey <key>

Safety
  The key is never printed.`)
	case len(path) >= 2 && path[0] == "person" && path[1] == "search":
		fmt.Println(`dipctl person search
...
Examples
  ...
  dipctl person search --name "Alice Weidel" --limit 3`)
	case len(path) >= 2 && path[0] == "person" && path[1] == "dossier":
		fmt.Println(`dipctl person dossier
...
Examples
  ...
  dipctl person dossier --name "Gauweiler"`)
	case len(path) >= 2 && path[0] == "vorgang" && path[1] == "dossier":
		fmt.Println(`dipctl vorgang dossier
...
Examples
  dipctl vorgang dossier --id 123456`)
	case len(path) >= 1 && path[0] == "source":
		fmt.Println(`dipctl source
...
Examples
  dipctl %s text --document-number "20/139" --grep "Buergergeld"
`, path[0], path[0])
	case len(path) >= 3 && path[0] == "plenary" && path[1] == "speech" && path[2] == "search":
		fmt.Println(`dipctl plenary speech search
...
Examples
  dipctl plenary speech search --document-number "20/139" --term "Buergergeld"
  dipctl plenary speech search --person "Gauweiler" --term "Arbeitslosigkeit"`)
	case len(path) >= 2 && isEntity(path[0]) && (path[1] == "list" || path[1] == "get"):
		fmt.Printf(`dipctl %s %s
...
Examples
  dipctl %s list --param "f.person=Gauweiler"
  dipctl %s get --id 760
...
func runPersonSearch(args []string) {
	...
	writeJSON(envelope{
		...
		"nextActions": []string{
			...
			"dipctl aktivitaet list --param \"f.person_id=<id>\"",
		},
	})
}
...
func runPersonDossier(args []string) {
	...
	writeJSON(envelope{
		...
		"nextActions": []string{
			"dipctl aktivitaet list --param \"f.person_id=" + id + "\"",
			"dipctl plenary speech search --person-id " + id + " --term <term>",
		},
	})
}
...
func runVorgangDossier(args []string) {
	...
	writeJSON(envelope{
		...
		"warnings": warnings,
		"nextActions": []string{
			"dipctl vorgangsposition list --param \"f.vorgang=" + id + "\"",
			"dipctl source --type vorgang --id " + id,
		},
	})
}
...
func runDocumentText(kind string, args []string) {
	...
	out := envelope{
		...
		"nextActions": []string{
			"dipctl source --type " + kind + " --id " + firstString(record, "id"),
		},
	}
	if grep != "" {
		out["snippets"] = snips
...
func runPlenarySpeechSearch(args []string) {
	...
	writeJSON(envelope{
		...
		"nextActions": []string{
			"dipctl plenarprotokoll text --document-number <number> --grep \"" + term + "\"",
		},
	})
}
