...
def print_root_help():
    print("""dip-bundestag -- official Bundestag DIP research CLI
...
def print_help_for(path):
    if path[:2] == ["person", "dossier"]:
        print("""dip-bundestag person dossier
...
def run_person_dossier(args):
    ...
    write_json({
        ...
        "warnings": warnings,
        "nextActions": [
            f'dip-bundestag aktivitaet list --param "f.person_id={record_id}"',
            f"dip-bundestag plenary speech search --person-id {record_id} --term <term>",
        ],
    })
...
def run_vorgang_dossier(args):
    ...
    write_json({
        ...
        "warnings": warnings,
        "nextActions": [f'dip-bundestag vorgangsposition list --param "f.vorgang={record_id}"'],
    })
...
def run_document_text(kind, args):
    ...
    out = {
        ...
        "nextActions": ["dip-bundestag source --type " + kind + " --id " + record_id],
    }
    if term:
...
def run_plenary_speech_search(args):
    ...
    write_json({
        ...
        "nextActions": [f'dip-bundestag plenarprotokoll text --document-number <number> --grep "{term}"'],
    })
