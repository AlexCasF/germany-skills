...
async function main(argv: string[]): Promise<number> {
  ...
  try {
    ...
    else if (matches(argv, "plenum", "speaker")) await runPlenumSpeaker(argv.slice(2));
    else if (matches(argv, "plenum", "conferences") || matches(argv, "plenum", "agenda")) await runPlenumConferences(argv.slice(2));
    else if (matches(argv, "members", "list")) await runMembersList(argv.slice(2));
    else if (matches(argv, "members", "search")) await runMembersSearch(argv.slice(2));
    else if (matches(argv, "members", "biography")) await runMemberBiography(argv.slice(2));
    else if (matches(argv, "members", "dossier")) await runMemberDossier(argv.slice(2));
    else if (matches(argv, "committees", "list")) await runCommitteesList(argv.slice(2));
    else if (matches(argv, "committees", "search")) await runCommitteesSearch(argv.slice(2));
    else if (matches(argv, "committees", "get") || matches(argv, "committees", "dossier")) await runCommitteeDossier(argv.slice(2));
    else if (matches(argv, "article", "get")) await runArticleGet(argv.slice(2));
    else if (matches(argv, "article", "page")) await runArticlePage(argv.slice(2));
    else if (matches(argv, "video", "feed")) await runVideoFeed(argv.slice(2));
    else if (argv[0] === "source") runSource(argv.slice(1));
    else throw new CLIError(2, "unknown_command", "unknown command; run bundestag-live --help");
  } catch (error) {
...
function printHelp(path: string[]): void {
  ...
  if (joined === "members search") console.log('bundestag-live members search --name "Mustername" --limit 3');
  else if (joined === "members dossier") console.log('bundestag-live members dossier --id 2022 --grep "Suchbegriff"');
  else if (joined === "committees dossier") console.log("bundestag-live committees dossier --id a11 --member-limit 5 --news-limit 3");
  else if (joined === "article page") console.log('bundestag-live article page --url "https://www.bundestag.de/..." --grep "term"');
  else printRootHelp();
}
...
async function runMembersList(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const { body, requestUrl } = await fetchXmlWithParams(MEMBERS_URL, parsed.params);
  if (flagBool(parsed, "raw")) return printRaw(body);
...
async function runMembersSearch(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const term = firstNonEmpty(parsed.flags.name, parsed.flags.term, parsed.flags.q, parsed.positionals.join(" "));
  if (!term) throw new CLIError(2, "missing_term", "members search requires --name or --term");
  const limit = limitFlag(parsed, DEFAULT_LIMIT, SAFE_LIMIT);
...
async function runMemberBiography(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const id = firstNonEmpty(parsed.flags.id, parsed.positionals[0]);
  if (!id) throw new CLIError(2, "missing_id", "members biography requires --id");
...
async function runMemberDossier(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  let id = parsed.flags.id ?? "";
  let resolved: JsonObject | null = null;
  if (!id) {
    const name = firstNonEmpty(parsed.flags.name, parsed.flags.term, parsed.positionals.join(" "));
    if (!name) throw new CLIError(2, "missing_member", "members dossier requires --id or --name");
  ...
  if (resolved) payload.resolvedFromIndex = compactMember(resolved, false);
  if (flagBool(parsed, "include-raw")) payload.rawXml = body;
  emit(payload);
}

async function runCommitteesList(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
...
async function runCommitteesSearch(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const term = firstNonEmpty(parsed.flags.term, parsed.flags.q, parsed.flags.name, parsed.positionals.join(" "));
  if (!term) throw new CLIError(2, "missing_term", "committees search requires --term");
  const limit = limitFlag(parsed, DEFAULT_LIMIT, SAFE_LIMIT);
...
async function runCommitteeDossier(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const id = firstNonEmpty(parsed.flags.id, parsed.positionals[0]);
  if (!id) throw new CLIError(2, "missing_id", "committees dossier requires --id");
  ...
  if (tag(body, "ausschussSourceURL")) payload.sources.push({ title: "Bundestag committee page", url: tag(body, "ausschussSourceURL"), kind: "public_page" });
  payload.warnings = defaultWarnings();
  payload.nextActions = [`bundestag-live committees dossier --id ${id} --member-limit 5`];
  emit(payload);
}

async function runPlenumSpeaker(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
...
async function runPlenumConferences(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  ...
  const days = dayBlocks.slice(0, limit).map((day) => {
    ...
    const items = itemBlocks.slice(0, itemLimit).map((item) => {
      ...
      if (articleId && nextActions.length < 3) nextActions.push(`bundestag-live article get --article-id ${articleId}`);
...
async function runArticleGet(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  let id = firstNonEmpty(parsed.flags["article-id"], parsed.flags.id, parsed.positionals[0]);
  if (!id && parsed.flags.url) id = articleIdFromUrl(parsed.flags.url);
  if (!id) throw new CLIError(2, "missing_article_id", "article get requires --article-id or --url");
...
async function runArticlePage(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  ...
  if (!sourceUrl) throw new CLIError(2, "missing_url", "article page requires --url or --article-id");
  if (!sourceUrl.startsWith(BASE_URL)) throw new CLIError(2, "unsafe_url", "article page only accepts www.bundestag.de URLs");
...
async function runVideoFeed(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  ...
  if (contentId && !params.contentId) params.contentId = contentId;
...
function runSource(argv: string[]): void {
  const parsed = parseArgs(argv);
  const sourceUrl = firstNonEmpty(parsed.flags.url, parsed.positionals[0]);
  if (!sourceUrl) throw new CLIError(2, "missing_url", "source requires --url");
...
function parseMembers(xml: string): JsonObject[] {
...
function compactMember(item: JsonObject, includeRaw: boolean): JsonObject {
  const out: JsonObject = { id: item.id, status: item.status, name: item.name, fraction: item.fraction, state: item.state, constituency: item.constituency, electionType: item.electionType, bioUrl: item.bioUrl, infoXmlUrl: item.infoXmlUrl, lastChanged: item.lastChanged };
  ...
  if (includeRaw) out.raw = item.raw;
  return out;
}

function memberSearchText(item: JsonObject): string {
  return [item.id, item.name, item.fraction, item.state, item.electionType, item.bioUrl, JSON.stringify(item.constituency)].join(" ");
}

async function resolveMember(term: string): Promise<JsonObject> {
  ...
  if (!found) throw new CLIError(2, "member_not_found", `member not found: ${term}`);
  return found;
}

function compactBiography(xml: string, grep: string): JsonObject {
...
function memberEvidence(xml: string, grep: string): JsonObject {
  const info = block(xml, "mdbInfo") || xml;
  const media = block(xml, "mdbMedien");
...
function sourcesForMember(xml: string, requestUrl: string): JsonObject[] {
  const info = block(xml, "mdbInfo") || xml;
  const media = block(xml, "mdbMedien");
  const out: JsonObject[] = [{ title: "Bundestag member biography XML", url: requestUrl, kind: "api_endpoint" }];
  const profile = firstNonEmpty(tag(info, "sourceURL"), tag(info, "mdbBioURL"));
  if (profile) out.push({ title: "Bundestag member profile", url: profile, kind: "public_profile" });
  if (tag(media, "mdbRedenVorPlenumURL")) out.push({ title: "Bundestag mediathek speeches filter", url: tag(media, "mdbRedenVorPlenumURL"), kind: "media_search" });
  if (tag(media, "mdbRedenVorPlenumRSS")) out.push({ title: "Bundestag speeches RSS", url: tag(media, "mdbRedenVorPlenumRSS"), kind: "rss" });
  return out;
}

function parseCommittees(xml: string): JsonObject[] {
...
function compactCommittee(item: JsonObject, includeRaw: boolean): JsonObject {
  const out: JsonObject = { id: item.id, name: item.name, shortName: item.shortName, teaser: item.teaser, detailXmlUrl: item.detailXmlUrl, imageUrl: item.imageUrl, imageSource: item.imageSource, lastChanged: item.lastChanged };
...
function committeeSearchText(item: JsonObject): string {
  return [item.id, item.name, item.shortName, item.teaser, item.detailXmlUrl].join(" ");
}

function parseCommitteeMembers(xml: string): JsonObject[] {
...
function parseCommitteeNews(xml: string, grep: string): JsonObject[] {
  ...
  return [...outer.matchAll(/<news\s+articleId="([^"]*)"[^>]*>([\s\S]*?)<\/news>/g)].map((match) => {
    ...
    if (grep && !`${text} ${tag(item, "title")}`.toLowerCase().includes(grep.toLowerCase())) return null;
...
function compactArticle(xml: string, grep: string): JsonObject {
...
function articleEvidence(xml: string, grep: string): JsonObject {
  const text = stripHtml(tag(xml, "text"));
...
async function fetchXmlWithParams(base: string, params: Record<string, string>): Promise<{ body: string; requestUrl: string }> {
...
function tagAttr(xml: string, name: string, attrName: string): string {
...
function printRaw(value: string): void {
  process.stdout.write(value);
}

function source(title: string, url: string, kind: string): JsonObject[] {
...
function sourceKind(url: string): string {
  if (url.includes("/xml/")) return "api_endpoint";
  if (url.includes("webtv.bundestag.de")) return "media_feed";
  if (url.includes("/mediathek")) return "media_page";
  if (url.includes("/abgeordnete/")) return "public_profile";
  return "public_page";
}
...
function grepSnippets(text: string, grep: string, limit: number, maxLen: number): JsonObject[] {
  text = stripSpace(text);
  if (!text) return [];
  if (!grep) return [{ text: truncate(text, maxLen) }];
...
function htmlTitle(value: string): string {
