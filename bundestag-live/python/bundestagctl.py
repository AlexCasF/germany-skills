...
def main(argv):
    ...
    try:
        if argv[0] == "doctor":
            ...
            raise CLIError(2, "unknown_command", "unknown command; run bundestagctl --help")
...
def print_root_help():
    print("""bundestagctl -- Bundestag live/site XML research CLI
...
def print_help(path):
    ...
    if joined == "members search":
        ...
        print('bundestagctl article page --url "https://www.bundestag.de/..." --grep "term"')
    else:
        print_root_help()


def print_examples():
    print("""bundestagctl examples
...
def run_members_search(argv):
    ...
    payload["sources"] = source("Bundestag member XML index", MEMBERS_URL, "api_endpoint")
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"bundestagctl members dossier --id {item['id']}" for item in matches[:3]] or ['bundestagctl members search --name "Amthor" --limit 3']
    emit(payload)
...
def run_member_dossier(argv):
    ...
    payload["nextActions"] = [f"bundestagctl members biography --id {member_id} --raw"]
    if resolved:
...
def run_committees_search(argv):
    ...
    payload["sources"] = source("Bundestag committee XML index", COMMITTEES_URL, "api_endpoint")
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"bundestagctl committees dossier --id {item['id']} --member-limit 5" for item in matches[:3]] or ["bundestagctl committees search --term Arbeit --limit 5"]
    emit(payload)
...
def run_committee_dossier(argv):
    ...
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"bundestagctl committees dossier --id {committee_id} --member-limit 5"]
    emit(payload)
...
def run_plenum_conferences(argv):
    ...
    for day in root.findall("./tagesordnung")[:limit]:
        ...
        for item in all_items[:item_limit]:
            ...
            if article_id and len(next_actions) < 3:
                next_actions.append(f"bundestagctl article get --article-id {article_id}")
            agenda.append({"startTime": find_text(item, "startzeit"), "endTime": find_text(item, "endzeit"), "status": find_text(item, "status"), "title": find_text(item, "titel"), "articleId": article_id, "top": find_text(item, "top"), "nextActions": [f"bundestagctl article get --article-id {article_id}"] if article_id else []})
    ...
    payload["nextActions"] = next_actions or ["bundestagctl plenum speaker"]
    emit(payload)
...
def run_article_get(argv):
    ...
    if find_text(root, "sourceURL"):
        ...
        payload["nextActions"] = [f"bundestagctl article page --url \"{find_text(root, 'sourceURL')}\""]
    payload["warnings"] = default_warnings()
    emit(payload)
...
def run_article_page(argv):
    ...
    payload["nextActions"] = [f"bundestagctl article get --article-id {article_id}"] if article_id else []
    emit(payload)
...
def run_source(argv):
    ...
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"bundestagctl article get --article-id {article_id_from_url(source_url)}"] if article_id_from_url(source_url) else []
    emit(payload)
...
def compact_member(item, include_raw):
    ...
    out["nextActions"] = [f"bundestagctl members dossier --id {item.get('id')}", f"bundestagctl members biography --id {item.get('id')} --raw"]
    if include_raw:
...
def compact_committee(item, include_raw):
    ...
    out["nextActions"] = [f"bundestagctl committees dossier --id {item.get('id')} --member-limit 5"]
    if include_raw:
        out["raw"] = item.get("raw")
    return out
