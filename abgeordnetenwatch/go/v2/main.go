...
func printRootHelp() {
	fmt.Println(`abgeordnetenwatchctl -- abgeordnetenwatch.de public transparency data
...
func printHelp(path []string) {
	...
	case "politicians dossier":
		fmt.Println(`abgeordnetenwatchctl politicians dossier
...
Next action
  Cross-check official parliamentary claims with DIP/Bundestag tools when needed.`)
	case "politicians page":
		fmt.Println(`abgeordnetenwatchctl politicians page
...
func runPoliticianSource(args []string) error {
	...
	payload["nextActions"] = []string{
		fmt.Sprintf(`abgeordnetenwatchctl politicians page --id %v`, record["id"]),
		fmt.Sprintf(`abgeordnetenwatchctl politicians dossier --id %v`, record["id"]),
	}
	emit(payload)
	return nil
}
...
func runPoliticianPage(args []string) error {
	...
	payload["warnings"] = standardWarnings()
	payload["nextActions"] = []string{fmt.Sprintf(`abgeordnetenwatchctl politicians dossier --id %v`, record["id"])}
	emit(payload)
	return nil
}
...
func runMandatesForPolitician(args []string) error {
	...
	payload["warnings"] = standardWarnings()
	payload["nextActions"] = []string{fmt.Sprintf(`abgeordnetenwatchctl sidejobs for-politician --id %s`, id)}
	emit(payload)
	return nil
}
