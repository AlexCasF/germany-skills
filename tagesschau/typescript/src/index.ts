...
async function main(argv: string[]): Promise<number> {
  try {
    ...
    else if (argv[0] === "homepage") await runFeed("homepage", HOMEPAGE_URL, argv.slice(1));
    else if (argv[0] === "news") await runFeed("news", NEWS_URL, argv.slice(1));
    else if (argv[0] === "channels") await runFeed("channels", CHANNELS_URL, argv.slice(1));
    else if (argv[0] === "search") await runSearch(argv.slice(1));
    else if (match(argv, "article", "get")) await runArticle("article get", argv.slice(2), false);
    else if (match(argv, "article", "source")) runArticleSource(argv.slice(2));
    else if (match(argv, "article", "dossier")) await runArticle("article dossier", argv.slice(2), true);
...
function printHelp(args: string[]): void {
  ...
  else if (args[0] === "search") console.log("search flags: --text/--searchText TERM --limit 1-30 --result-page N --include-raw --raw --param key=value");
  else if (args[0] === "news") console.log("news flags: --ressort inland|ausland|wirtschaft|sport|video|investigativ|wissen --regions 1,2 --limit 1-30 --include-raw --raw --param key=value");
  else if (args[0] === "homepage") console.log("homepage flags: --limit 1-30 --include-regional --include-raw --raw");
  else if (match(args, "article", "get")) console.log("article get flags: --url URL --grep TERM --limit 1-30 --include-raw --raw");
  else if (match(args, "article", "source")) console.log("article source flags: --url URL");
  else if (match(args, "article", "dossier")) console.log("article dossier flags: --url URL --grep TERM --limit 1-30 --include-raw");
  else printRootHelp();
}
...
async function runFeed(command: string, endpoint: string, argv: string[]): Promise<void> {
  ...
  if (response.status < 200 || response.status >= 300) throw new CliError("upstream_http_error", `upstream status ${response.status} from ${requestUrl}: ${stripSpace(response.body).slice(0, 260)}`);
  if (flagBool(parsed, "raw")) {
    process.stdout.write(response.body);
    return;
  }
  const data = JSON.parse(response.body) as JsonObject;
...
async function runSearch(argv: string[]): Promise<void> {
  ...
  if (parsed.flags["page-size"] || parsed.flags.pageSize) params.pageSize = first(parsed.flags["page-size"], parsed.flags.pageSize);
  if (parsed.flags["result-page"] || parsed.flags.resultPage || parsed.flags.page) params.resultPage = first(parsed.flags["result-page"], parsed.flags.resultPage, parsed.flags.page);
...
async function runArticle(command: string, argv: string[], dossier: boolean): Promise<void> {
  const parsed = parseArgs(argv);
  const inputUrl = first(parsed.flags.url, parsed.params.url, parsed.positionals.join(" "));
  if (!inputUrl) throw new CliError("missing_url", `${command} requires --url`, 2);
  const urls = articleUrls(inputUrl);
  const response = await fetchRaw(urls.apiUrl);
  if (response.status < 200 || response.status >= 300) throw new CliError("upstream_http_error", `upstream status ${response.status} from ${urls.apiUrl}: ${stripSpace(response.body).slice(0, 260)}`);
  if (flagBool(parsed, "raw")) {
    process.stdout.write(response.body);
    return;
  }
  ...
  if (dossier) summary.dossierUse = "Use as current-news context; verify institutional/statistical claims against primary official sources.";
...
function runArticleSource(argv: string[]): void {
  const parsed = parseArgs(argv);
  const inputUrl = first(parsed.flags.url, parsed.params.url, parsed.positionals.join(" "));
  if (!inputUrl) throw new CliError("missing_url", "article source requires --url", 2);
  const urls = articleUrls(inputUrl);
...
function compactFeedItems(data: JsonObject, limit: number, includeRegional: boolean): JsonObject[] {
  const items = compactArray(data.news, limit);
  if (includeRegional && items.length < limit) items.push(...compactArray(data.regional, limit - items.length));
  if (items.length === 0) items.push(...compactArray(data.channels, limit));
  return items.slice(0, limit);
}

function compactArray(value: unknown, limit: number): JsonObject[] {
  return Array.isArray(value) ? value.filter(isObject).slice(0, limit).map(compactArticle) : [];
}

function compactArticle(obj: JsonObject): JsonObject {
  ...
  if (!publicUrl && details) publicUrl = articleUrls(details).publicUrl;
...
function articleSnippets(data: JsonObject, grep: string, limit: number): JsonObject[] {
  if (!Array.isArray(data.content)) return [];
  const needle = grep.toLowerCase();
  const snippets: JsonObject[] = [];
  data.content.forEach((raw, index) => {
    if (snippets.length >= limit || !isObject(raw)) return;
    const type = stringValue(raw.type);
    if (type !== "text" && type !== "headline") return;
    const text = stripHtml(stringValue(raw.value));
    if (!text || (needle && !text.toLowerCase().includes(needle))) return;
    snippets.push({ index, type, text: truncate(text, 520), matched: !needle || text.toLowerCase().includes(needle) });
  });
  return snippets;
}

function articleUrls(inputUrl: string): { apiUrl: string; publicUrl: string } {
  let parsed: URL;
  try {
  ...
  if (!parsed.hostname.endsWith("tagesschau.de")) throw new CliError("invalid_url", "expected a tagesschau.de URL", 2);
...
async function fetchRaw(requestUrl: string): Promise<{ status: number; body: string; contentType: string }> {
  let lastStatus = 0;
  let lastBody = "";
  let lastContentType = "";
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      if (![429, 502, 503, 504].includes(response.status)) return { status: response.status, body, contentType: response.headers.get("content-type") ?? "" };
  ...
  if (lastStatus) return { status: lastStatus, body: lastBody, contentType: lastContentType };
...
function envelope(command: string, method: string, requestUrl: string, params: unknown): JsonObject {
...
function feedCounts(data: JsonObject): Record<string, number> {
...
function limitFlag(parsed: ParsedArgs): number {
  if (!parsed.flags.limit) return DEFAULT_LIMIT;
  const value = Number(parsed.flags.limit);
  if (!Number.isInteger(value)) throw new CliError("invalid_limit", "--limit must be an integer", 2);
