...
func printRootHelp() {
	fmt.Println(`tagesschauctl -- Tagesschau public JSON feed research CLI
...
func printExamples() {
  ...
  tagesschauctl article dossier --url "https://www.tagesschau.de/api2u/inland/example-100.json"`)
}
...
func runSource(argv []string) error {
	...
	payload["sources"] = defaultSources()
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{"tagesschauctl fields", "tagesschauctl search --text \"Bundestag\" --limit 5"}
	emit(payload)
	return nil
}
...
func runFields(argv []string) error {
	...
	payload["sources"] = defaultSources()
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{"tagesschauctl search --text \"Bundestag\" --limit 5", "tagesschauctl homepage --limit 5"}
	emit(payload)
	return nil
}
...
func runArticle(command string, argv []string, dossier bool) error {
	...
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{
		fmt.Sprintf("tagesschauctl article source --url %q", publicURL),
	}
	if dossier {
		...
		payload["nextActions"] = append(payload["nextActions"].([]string), "tagesschauctl source")
	}
	if flagBool(parsed, "include-raw") {
		payload["raw"] = data
	}
...
func runArticleSource(argv []string) error {
	...
	payload["sources"] = append(payload["sources"].([]map[string]string), defaultSources()...)
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{fmt.Sprintf("tagesschauctl article get --url %q --limit 5", publicURL)}
	emit(payload)
	return nil
}
...
func compactArticle(obj map[string]any) map[string]any {
	...
	if publicURL != "" {
		...
		item["nextActions"] = []string{
			fmt.Sprintf("tagesschauctl article get --url %q --limit 5", publicURL),
			fmt.Sprintf("tagesschauctl article source --url %q", publicURL),
		}
	}
	return item
...
func nextActionsFromItems(items []map[string]any) []string {
	...
	for _, item := range items {
		if urlValue, ok := item["sourceUrl"].(string); ok && urlValue != "" {
			actions = append(actions, fmt.Sprintf("tagesschauctl article get --url %q --limit 5", urlValue))
		}
		if len(actions) >= 3 {
			break
		}
	...
	if len(actions) == 0 {
		actions = append(actions, "tagesschauctl source")
	}
	return actions
}
