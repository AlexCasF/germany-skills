...
def print_root_help() -> None:
    print(
        """tagesschauctl 2.0 - Tagesschau public JSON feed research CLI
...
def run_article(command: str, argv: list[str], dossier: bool) -> None:
    ...
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f'tagesschauctl article source --url "{public_url}"']
    if dossier:
        payload["nextActions"].append("tagesschauctl source")
    if flag_bool(parsed, "include-raw"):
        payload["raw"] = data
    emit(payload)
...
def run_article_source(argv: list[str]) -> None:
    ...
    payload["sources"] = [{"kind": "api_request", "title": "Tagesschau article JSON", "url": api_url}, {"kind": "public_article", "title": "Tagesschau public article", "url": public_url}] + default_sources()
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f'tagesschauctl article get --url "{public_url}" --limit 5']
    emit(payload)
...
def compact_article(obj: dict[str, Any]) -> dict[str, Any]:
    ...
    if public_url:
        ...
        item["nextActions"] = [f'tagesschauctl article get --url "{public_url}" --limit 5', f'tagesschauctl article source --url "{public_url}"']
    return item
...
def next_actions_from_items(items: list[dict[str, Any]]) -> list[str]:
    actions = [f'tagesschauctl article get --url "{item["sourceUrl"]}" --limit 5' for item in items[:3] if item.get("sourceUrl")]
    return actions or ["tagesschauctl source"]
