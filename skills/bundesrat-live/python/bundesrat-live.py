...
def main(argv):
    ...
    try:
        if argv[0] == "doctor":
            ...
            raise CLIError(2, "unknown_command", "unknown command; run bundesrat-live --help")
...
def print_root_help():
    print("""bundesrat-live -- Bundesrat live/app XML research CLI
...
def print_help(path):
    ...
    if joined in {"news search", "dates search"}:
        ...
        print('bundesrat-live members dossier --name "Ã–zdemir" --grep "Bundesrat"')
    elif joined in {"page", "news page", "dates page"}:
        print('bundesrat-live page --url "https://www.bundesrat.de/..." --grep "term"')
    else:
        print_root_help()


def print_examples():
    print("""bundesrat-live examples
...
def run_member_dossier(argv):
    ...
    if item.get("url"):
        ...
        payload["nextActions"] = [f'bundesrat-live page --url "{item["url"]}" --grep "{first_non_empty(grep, "Bundesrat")}"']
    payload["warnings"] = default_warnings()
    emit(payload)
...
def run_page(command, argv):
    ...
    payload["nextActions"] = [f'bundesrat-live source --url "{source_url}"']
    emit(payload)
...
def run_source(argv):
    ...
    if source_url.startswith(BASE_URL + "/"):
        payload["nextActions"] = [f'bundesrat-live page --url "{source_url}"']
    emit(payload)
...
def next_actions_from_employees(items):
    actions = [f'bundesrat-live members dossier --name "{item.get("name")}"' for item in items[:3] if item.get("name")]
    return actions or ['bundesrat-live members search --name "Ã–zdemir" --limit 3']
...
def next_actions_for_url(source_url, key):
    ...
    actions = []
    if source_url.startswith(BASE_URL + "/"):
        actions.append(f'bundesrat-live page --url "{source_url}"')
    if key == "news":
        actions.append(f'bundesrat-live news page --url "{source_url}"')
    if key == "dates":
        actions.append(f'bundesrat-live dates page --url "{source_url}"')
    return actions
