...
async function runDoctor(argv: string[]): Promise<void> {
  ...
  try {
    ...
    (payload.request as JsonObject).url = requestUrl;
    (payload.summary as JsonObject).health = {
      ok: true,
...
async function runStatistics(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
...
async function runSearch(argv: string[]): Promise<void> {
  ...
  if (!term) throw new CLIError(2, "missing_term", "search requires --term, --q, or --name");
  const limit = limitFlag(parsed, 5, 25);
  const params: Record<string, string> = { format: "json", q: term };
  if (parsed.flags.cursor) params.cursor = parsed.flags.cursor;
...
async function runEntryGet(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  ...
  if (flagBool(parsed, "include-raw")) payload.raw = entry;
  emit(payload);
}

async function runEntrySource(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const { entry, requestUrl } = await getEntryFromArgs(parsed);
...
async function runEntryDossier(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const { entry, requestUrl } = await getEntryFromArgs(parsed);
...
async function runFinancialSummary(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const { entry, requestUrl } = await getEntryFromArgs(parsed);
...
async function runStatementsList(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const { entry, requestUrl } = await getEntryFromArgs(parsed);
...
async function runRawSearch(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const params = new URLSearchParams(parsed.params);
  for (const [key, value] of Object.entries(parsed.flags)) {
    if (!["include-raw", "timeout"].includes(key)) params.set(key, value);
  }
...
async function getEntryFromArgs(parsed: ParsedArgs): Promise<{ entry: JsonObject; requestUrl: string }> {
  const key = requireKey(parsed);
...
async function searchFirst(term: string, key: string): Promise<{ entry: JsonObject; requestUrl: string }> {
  const { data, requestUrl } = await apiJson("/registerentries", { format: "json", q: term }, key);
  const results = Array.isArray(data.results) ? data.results : [];
  if (results.length === 0) throw new CLIError(1, "not_found", "no register entry found for name: " + term);
  return { entry: asObject(results[0]), requestUrl };
}

async function apiJson(path: string, params: Record<string, string>, key: string): Promise<{ data: JsonObject; requestUrl: string }> {
  if (!params.format) params.format = "json";
  const url = new URL(BASE_URL + path);
  for (const [paramKey, value] of Object.entries(params)) url.searchParams.set(paramKey, value);
  const body = await httpGetText(url.toString(), {
    Authorization: "ApiKey " + key,
    Accept: "application/json"
  });
  return { data: JSON.parse(body) as JsonObject, requestUrl: sanitizeUrl(url.toString()) };
}

function httpGetText(rawUrl: string, headers: Record<string, string>): Promise<string> {
...
function summarizeEntry(entry: JsonObject): JsonObject {
  return {
    registerNumber: entry.registerNumber,
    name: get(entry, "lobbyistIdentity", "name"),
...
function financialBlock(entry: JsonObject): JsonObject {
  return {
    financialExpenses: {
...
function compactProjects(entry: JsonObject, limit: number): JsonObject[] {
...
function compactStatements(entry: JsonObject, grep: string, limit: number): JsonObject[] {
...
function entrySources(entry: JsonObject): JsonObject[] {
...
function addSource(sources: JsonObject[], title: string, url: string, kind: string): void {
  if (url) sources.push({ title, url, kind });
}
...
function nextActionsForEntry(entry: JsonObject): string[] {
...
function searchNextActions(items: JsonObject[]): string[] {
  return items
...
function requireKey(parsed: ParsedArgs): string {
  const key = apiKey(parsed);
  if (!key) throw new CLIError(2, "missing_api_key", "set LOBBYREGISTER_API_KEY or pass --apikey");
  return key;
}

function apiKey(parsed: ParsedArgs): string {
  return parsed.flags.apikey || process.env.LOBBYREGISTER_API_KEY || "";
}

function keySource(parsed: ParsedArgs): string {
  if (parsed.flags.apikey) return "flag:redacted";
  if (process.env.LOBBYREGISTER_API_KEY) return "env:LOBBYREGISTER_API_KEY";
  return "missing";
}

function envelope(command: string, requestUrl: string): JsonObject {
  return {
    status: "ok",
    tool: APP_NAME,
    command,
...
function labelsFromArray(items: unknown[], limit: number): string[] {
  const labels: string[] = [];
...
function compactNamedItems(items: unknown[], limit: number): JsonObject[] {
...
function fiscalYear(entry: JsonObject, block: string): JsonObject {
  return {
