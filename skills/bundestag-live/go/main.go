...
func main() {
	...
	default:
		err = cliError{2, "unknown_command", "unknown command; run bundestag-live --help"}
	}
	if err != nil {
		var ce cliError
		if errors.As(err, &ce) {
...
func printRootHelp() {
	fmt.Println(`bundestag-live -- Bundestag live/site XML research CLI
...
Do not use this when
  - you need complete parliamentary proceedings, printed papers, or plenary
    protocol history; use dip-bundestag for that archive-grade research
  - you need Bundesrat proceedings; use bundesrat-live
...
func printHelp(path []string) {
	...
	case "members search":
		fmt.Println(`bundestag-live members search
...
Flags
  ...
  --include-raw       Include the matching raw member records`)
	case "members dossier":
		fmt.Println(`bundestag-live members dossier
...
Flags
  ...
  --include-raw       Include raw parsed biography fields`)
	case "committees dossier":
		fmt.Println(`bundestag-live committees dossier
...
Flags
  ...
  --grep <term>       Return snippets from task/news text`)
	case "article page":
		fmt.Println(`bundestag-live article page

Fetch and normalize a human-facing Bundestag article page or article XML ID.

Examples
  bundestag-live article page --url "https://www.bundestag.de/dokumente/textarchiv/2026/kw21-de-demokratie-1174778"
  bundestag-live article page --article-id 1174778 --grep "Meinungsfreiheit"
...
Flags
  ...
  --grep <term>       Return source snippets matching term`)
	case "plenum conferences":
		fmt.Println(`bundestag-live plenum conferences

Normalize the Bundestag live plenary conference overview.

Examples
  bundestag-live plenum conferences --limit 2 --item-limit 3
  bundestag-live plenum conferences --raw
...
func printExamples() {
	fmt.Println(`bundestag-live examples
...
9. Fetch raw XML when you need the complete upstream record:
   bundestag-live members biography --id 2022 --raw

10. Prefer dip-bundestag for full parliamentary proceedings and historical plenary protocols.`)
}
...
func runMemberDossier(argv []string) error {
	...
	payload["nextActions"] = []string{
		fmt.Sprintf("bundestag-live members biography --id %s --raw", id),
		fmt.Sprintf("bundestag-live source --url %q", firstNonEmpty(bio.Info.SourceURL, bio.Info.BioURL)),
	}
	if resolved != nil {
...
func runCommitteesList(argv []string) error {
	...
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{"bundestag-live committees search --term Arbeit --limit 5"}
	emit(payload)
	return nil
}
...
func runCommitteeGet(argv []string) error {
	...
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{fmt.Sprintf("bundestag-live committees dossier --id %s --member-limit 5", id)}
	if flagBool(parsed, "include-raw") {
		payload["raw"] = detail
	}
	emit(payload)
...
func runPlenumSpeaker(argv []string) error {
	...
	payload["nextActions"] = []string{"bundestag-live plenum conferences --limit 2 --item-limit 5"}
	if flagBool(parsed, "include-raw") {
...
func runArticleGet(argv []string) error {
	...
	if article.SourceURL != "" {
		payload["nextActions"] = append(payload["nextActions"].([]string), fmt.Sprintf("bundestag-live article page --url %q", article.SourceURL))
	}
	if flagBool(parsed, "include-raw") {
		payload["raw"] = article
	}
...
func runArticlePage(argv []string) error {
	...
	if articleID := articleIDFromURL(sourceURL); articleID != "" {
		payload["nextActions"] = []string{fmt.Sprintf("bundestag-live article get --article-id %s", articleID)}
	}
	emit(payload)
	return nil
}
...
func compactMember(member memberListItem, includeRaw bool) map[string]any {
	item := map[string]any{
		...
		"nextActions": []string{
			fmt.Sprintf("bundestag-live members dossier --id %s", member.ID.Text),
			fmt.Sprintf("bundestag-live members biography --id %s --raw", member.ID.Text),
		},
	}
	if includeRaw {
		item["raw"] = member
...
func compactCommittees(committees []committeeListItem, limit int, includeRaw bool) []map[string]any {
	...
	for _, committee := range committees[:minInt(limit, len(committees))] {
		item := map[string]any{
			...
			"nextActions":  []string{fmt.Sprintf("bundestag-live committees dossier --id %s --member-limit 5", committee.ID)},
		}
		if includeRaw {
			item["raw"] = committee
		}
...
func compactCommitteeMembers(members []committeeMemberItem, limit int) []map[string]any {
	...
	for _, member := range members[:minInt(limit, len(members))] {
		out = append(out, map[string]any{
			...
			"infoXmlUrl":  member.InfoXMLURL,
			"lastChanged": member.LastChanged,
			"nextActions": []string{fmt.Sprintf("bundestag-live members dossier --id %s", member.ID.Text)},
		})
	}
	return out
}
...
func compactCommitteeNews(news []committeeNewsItem, limit int, grep string) []map[string]any {
	...
	for _, item := range news[:minInt(limit, len(news))] {
		...
		out = append(out, map[string]any{
			...
			"nextActions":     []string{fmt.Sprintf("bundestag-live article get --article-id %s", item.ArticleID)},
		})
	}
	return out
}
...
func defaultWarnings() []string {
	return []string{
		...
		"This live/site XML surface is not the full parliamentary archive. Use dip-bundestag for complete proceedings, printed papers, and plenary protocol research.",
...
func nextActionsForCommittees(committees []committeeListItem) []string {
	...
	for _, committee := range committees[:minInt(3, len(committees))] {
		actions = append(actions, fmt.Sprintf("bundestag-live committees dossier --id %s --member-limit 5", committee.ID))
	}
	if len(actions) == 0 {
		return []string{"bundestag-live committees search --term Arbeit --limit 5"}
	}
	return actions
}
...
func nextActionsForConferenceDays(days []conferenceDay) []string {
	...
	for _, day := range days {
		for _, item := range day.Items {
			if item.ArticleID != "" {
				actions = append(actions, fmt.Sprintf("bundestag-live article get --article-id %s", item.ArticleID))
			}
			if len(actions) >= 3 {
				return actions
			}
	...
	return []string{"bundestag-live plenum speaker"}
}
...
func nextActionsForArticleID(articleID string) []string {
	...
	return []string{fmt.Sprintf("bundestag-live article get --article-id %s", articleID)}
}

func nextActionsForSourceURL(sourceURL string) []string {
	if strings.Contains(sourceURL, "/xml/v2/mdb/biografien/") {
		return []string{fmt.Sprintf("bundestag-live members biography --id %s", articleIDFromURL(sourceURL))}
	}
	if id := articleIDFromURL(sourceURL); id != "" {
		return []string{fmt.Sprintf("bundestag-live article get --article-id %s", id), fmt.Sprintf("bundestag-live article page --url %q", sourceURL)}
	}
	return []string{}
}
