...
def print_root_help():
    print("""destatisctl -- Destatis GENESIS-Online statistics CLI
...
def print_help(path):
    ...
    if joined == "table dossier":
        print("""destatisctl table dossier
...
def run_table_source(argv):
    ...
    payload["nextActions"] = [f"destatisctl table dossier --name {name}", f"destatisctl metadata table --param name={name}"]
    emit(payload)
...
def run_table_dossier(argv):
    ...
    payload["sources"] = sources_for_table(name)
    payload["warnings"] = standard_warnings(cred)
    payload["nextActions"] = [f"destatisctl table sample --name {name}", f"destatisctl variables explain --table {name}"]
    try:
...
def run_variables_explain(argv):
    ...
    payload["nextActions"] = [f"destatisctl table dossier --name {table}"]
    emit(payload)
...
def next_actions_for_find(items):
    ...
    for item in items:
        if item.get("kind") == "table":
            actions.append(f"destatisctl table dossier --name {item['code']}")
        elif item.get("kind") == "timeseries":
            actions.append(f"destatisctl timeseries dossier --name {item['code']}")
        elif item.get("kind") == "statistic":
            actions.append(f"destatisctl catalogue tables --param name={item['code']}")
        if len(actions) >= 5:
            break
    return actions
