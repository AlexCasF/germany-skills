...
func printRootHelp() {
	fmt.Println(`destatisctl -- Destatis GENESIS-Online statistics CLI
...
Legacy endpoint commands
  destatisctl catalogue statistics
  destatisctl catalogue tables
  destatisctl catalogue variables
  destatisctl metadata table
  destatisctl metadata timeseries
  destatisctl data table
  destatisctl data timeseries
  destatisctl find search
...
func printHelp(path []string) {
	...
	case "table dossier":
		fmt.Println(`destatisctl table dossier
...
Examples
  destatisctl table dossier --name 12211-0900
  destatisctl table dossier --name 12211-0900 --include-raw`)
	case "search":
		fmt.Println(`destatisctl search
...
func runTableSource(argv []string) error {
	...
	payload["nextActions"] = []string{
		"destatisctl table dossier --name " + name,
		"destatisctl metadata table --param name=" + name,
	}
	emit(payload)
	return nil
}
...
func runTableDossier(argv []string) error {
	...
	payload["warnings"] = standardWarnings(cred)
	payload["nextActions"] = []string{
		"destatisctl table sample --name " + name,
		"destatisctl variables explain --table " + name,
	}
...
func runVariablesExplain(argv []string) error {
	...
	payload["nextActions"] = []string{"destatisctl table dossier --name " + table}
	emit(payload)
	return nil
}
...
func nextActionsForFind(items []any) []string {
	...
	for _, item := range items {
		...
		case "table":
			actions = append(actions, "destatisctl table dossier --name "+code)
		case "timeseries":
			actions = append(actions, "destatisctl timeseries dossier --name "+code)
		case "statistic":
			actions = append(actions, "destatisctl catalogue tables --param name="+code)
		}
		if len(actions) >= 5 {
			break
		}
