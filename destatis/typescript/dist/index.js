...
async function main(argv) {
    ...
    try {
        ...
        else if (matches(argv, "table", "dossier"))
            await runTableDossier(argv.slice(2));
        else if (matches(argv, "table", "sample"))
            await runTableSample(argv.slice(2));
        else if (matches(argv, "timeseries", "dossier"))
            await runTimeseriesDossier(argv.slice(2));
...
async function runDoctor(argv) {
    ...
    try {
        ...
        payload.summary.health = { ok: true, message: login.Status, username: redactUsername(asString(login.Username)) };
    }
    catch (error) {
...
async function runSearch(argv) {
    ...
    if (!term)
        throw new CLIError(2, "missing_term", "search requires --term");
...
async function runTableDossier(argv) {
    ...
    try {
        ...
        if (flagBool(parsed, "include-raw"))
            payload.rawMetadata = metadata;
    }
    catch (error) {
        payload.metadata = { available: false, error: redact(error instanceof Error ? error.message : String(error)) };
        payload.warnings.push("Metadata request failed; guest credentials can be insufficient for metadata/data endpoints.");
    }
    if (flagBool(parsed, "sample")) {
        try {
            ...
            payload.warnings.push("Data sample request failed; use personal GENESIS credentials for protected data endpoints.");
        }
    }
    emit(payload);
}
...
async function runTimeseriesDossier(argv) {
    const parsed = parseArgs(argv);
    const cred = resolveCredentials(parsed);
    const name = requiredName(parsed);
...
async function runVariablesExplain(argv) {
    const parsed = parseArgs(argv);
    const cred = resolveCredentials(parsed);
    const table = firstNonEmpty(parsed.flags.table, parsed.flags.name, parsed.flags.code);
    if (!table)
        throw new CLIError(2, "missing_table", "variables explain requires --table");
...
async function runLegacy(argv) {
    ...
    for (const [key, value] of Object.entries(parsed.flags)) {
        if (!["username", "password", "limit", "include-raw", "sample"].includes(key))
            params[key] = value;
    }
    if (command === "find search" && !params.term && parsed.flags.selection)
...
async function apiPost(path, params, cred) {
    return JSON.parse(await apiPostText(path, params, cred));
}
async function apiPostText(path, params, cred) {
    ...
    if (!response.ok)
        throw new CLIError(1, "http_error", `HTTP ${response.status} from Destatis GENESIS API: ${truncate(text, 280)}`);
    return text;
}
function compactFind(data, limit) {
    const items = [];
    const add = (kind, key) => {
...
function summarizeDestatisPayload(data) {
    return {
        status: data.Status,
        ident: data.Ident,
...
function tableSourceSummary(name) {
    return {
        name,
        kind: "table",
        apiBaseUrl: BASE_URL,
...
function sourceLinks(kind, code) {
    if (kind === "table")
...
function standardWarnings(cred) {
    const warnings = [
    ...
    if (cred.guest)
        warnings.push("Using GAST/GAST fallback: discovery works, but metadata/data endpoints may return 401; configure DESTATIS_USERNAME and DESTATIS_PASSWORD for full access.");
    return warnings;
}
function nextActionsForFind(items) {
    const actions = [];
...
function resolveCredentials(parsed) {
    ...
    else if (process.env.DESTATIS_USERNAME || process.env.DESTATIS_PASSWORD)
        source = "env:DESTATIS_USERNAME/DESTATIS_PASSWORD";
    return { username, password, source, guest: username === "GAST" && password === "GAST" };
}
function envelope(command, path, params, cred) {
    const request = { method: "POST", url: BASE_URL + path, credentialSource: cred.source, redactedFields: ["username", "password"] };
    if (params)
        request.params = redactParamMap(params);
    return { status: "ok", tool: APP_NAME, command, retrievedAt: new Date().toISOString(), request };
}
function requiredName(parsed) {
    const name = firstNonEmpty(parsed.flags.name, parsed.flags.code, parsed.flags.table, parsed.positionals[0]);
    if (!name)
        throw new CLIError(2, "missing_name", "requires --name, --code, or --table");
    return name;
}
function redactParamMap(params) {
    const out = {};
