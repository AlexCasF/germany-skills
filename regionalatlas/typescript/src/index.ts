...
type FlatIndicator = { topic: string; node: JsonObject };
...
async function main(argv: string[]): Promise<number> {
  ...
  try {
    ...
    else if (matches(argv, "indicators", "list")) await runIndicatorsList(argv.slice(2));
    else if (matches(argv, "indicators", "search")) await runIndicatorsSearch(argv.slice(2));
    else if (matches(argv, "indicator", "get")) await runIndicatorGet(argv.slice(2));
    else if (argv[0] === "fields") await runFields(argv.slice(1));
    else if (argv[0] === "sample") await runSample(argv.slice(1));
    else if (argv[0] === "source") await runSource(argv.slice(1));
    else if (argv[0] === "dossier") await runDossier(argv.slice(1));
...
async function runIndicatorsList(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  ...
  if (topic) flat = flat.filter((item) => item.topic.toLowerCase().includes(topic));
...
async function runIndicatorsSearch(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const term = firstNonEmpty(parsed.flags.term, parsed.flags.q, parsed.positionals.join(" "));
  if (!term) throw new CLIError(2, "missing_term", "indicators search requires --term");
...
async function runFields(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const item = await findIndicator(requiredIndicator(parsed));
...
async function runSample(argv: string[]): Promise<void> {
  ...
  const warnings = defaultWarnings();
  if (data.exceededTransferLimit) warnings.push("ArcGIS reported exceededTransferLimit=true; the returned sample is not a complete extract.");
  if (flagBool(parsed, "geometry")) warnings.push("Geometry was requested intentionally; municipality-level geometry can be very large.");
...
async function runDossier(argv: string[]): Promise<void> {
  ...
  const requestUrl = `${QUERY_ENDPOINT}?${params.toString()}`;
  const warnings = defaultWarnings();
  let sampleData: JsonObject | undefined;
  try {
  ...
  if (sampleData) {
    ...
    if (sampleData.exceededTransferLimit) warnings.push("Sample query reports exceededTransferLimit=true; use pagination/filtering for complete extraction.");
  }
  payload.sources = sourcesForIndicator(item.node, field, year);
  payload.warnings = warnings;
  payload.nextActions = [
...
async function runExplainField(argv: string[]): Promise<void> {
  ...
  if (!attr) throw new CLIError(2, "field_not_found", "field not found in indicator attributes");
...
async function runRawQuery(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  ...
  if (parsed.flags["layer-file"]) {
    try {
      params.set("layer", readFileSync(parsed.flags["layer-file"], "utf8").replace(/^\uFEFF/, "").trim());
    } catch (error) {
      throw new CLIError(2, "layer_file_read_failed", error instanceof Error ? error.message : String(error));
    }
  }
  if (parsed.flags.layer) params.set("layer", parsed.flags.layer);
  if (!params.get("f")) params.set("f", "json");
  if (!params.get("returnGeometry") && !params.get("returngeometry")) params.set("returnGeometry", "false");
  if (!params.get("where")) params.set("where", "1=1");
  if (!params.get("spatialRel")) params.set("spatialRel", "esriSpatialRelIntersects");
  if (!params.get("resultRecordCount") && !params.get("resultrecordcount")) params.set("resultRecordCount", String(limitFlag(parsed, DEFAULT_LIMIT, SAFE_LIMIT)));
...
async function fetchCatalog(): Promise<any[]> {
  ...
  if (!Array.isArray(data)) throw new CLIError(1, "catalog_shape_changed", "Regionalatlas catalog is not an array");
  return data;
}
...
async function fetchJson(requestUrl: string): Promise<JsonObject> {
  ...
  if (!response.ok) throw new Error(`HTTP ${response.status} from ${requestUrl}: ${body.slice(0, 300)}`);
  const data = JSON.parse(body);
  if (data && typeof data === "object" && data.error) throw new Error(`upstream error ${JSON.stringify(data.error)}`);
  return data;
}

function requiredIndicator(parsed: ParsedArgs): string {
  const code = firstNonEmpty(parsed.flags.indicator, parsed.flags.code, parsed.flags.table, firstPosition(parsed));
  if (!code) throw new CLIError(2, "missing_indicator", "command requires --indicator");
  return code.toUpperCase();
}

async function findIndicator(code: string): Promise<FlatIndicator> {
  const norm = normalizeCode(code);
  for (const item of flattenCatalog(await fetchCatalog())) {
    if (normalizeCode(String(item.node.code ?? "")) === norm) return item;
  }
  throw new CLIError(2, "indicator_not_found", "indicator not found in Regionalatlas catalog");
}

async function resolveQueryInputs(parsed: ParsedArgs): Promise<{ item: FlatIndicator; field: string; year: number; regionLevel: number }> {
  const item = await findIndicator(requiredIndicator(parsed));
  const field = firstNonEmpty(parsed.flags.field, parsed.flags.icode, firstAttributeCode(item.node)).toLowerCase();
  if (!findAttribute(item.node, field)) throw new CLIError(2, "field_not_found", "field not found for indicator");
  const year = intFlag(parsed, "year", latestYear(item.node));
  if (!year) throw new CLIError(2, "missing_year", "year could not be inferred; pass --year");
  const regionLevel = intFlag(parsed, "region-level", intFlag(parsed, "typ", 1));
  if (![1, 2, 3, 5].includes(regionLevel)) throw new CLIError(2, "invalid_region_level", "region-level must be one of 1, 2, 3, or 5");
  return { item, field, year, regionLevel };
}

function buildQueryParams(node: JsonObject, field: string, year: number, regionLevel: number, limit: number, parsed: ParsedArgs): URLSearchParams {
  ...
  if (parsed.flags.ags && where === "1=1") where = `ags = '${parsed.flags.ags.replace(/'/g, "''")}'`;
  ...
  for (const [key, value] of Object.entries(parsed.params)) {
    if (key !== "layer") params.set(key, value);
  }
  return params;
}

function flattenCatalog(catalog: any[]): FlatIndicator[] {
  const flat: FlatIndicator[] = [];
  const walk = (nodes: any[], topic = ""): void => {
    for (const node of nodes ?? []) {
      let nextTopic = topic;
      if (node?.title && !node?.code) nextTopic = String(node.title);
      if (node?.code && node?.attributes) flat.push({ topic: nextTopic, node });
      if (Array.isArray(node?.children)) walk(node.children, nextTopic);
    }
  };
  walk(catalog);
...
function searchCatalog(flat: FlatIndicator[], term: string): FlatIndicator[] {
...
function compactIndicators(items: FlatIndicator[], limit: number): JsonObject[] {
...
function compactAttributes(attrs: JsonObject[], limit: number): JsonObject[] {
...
function compactFeatures(data: JsonObject, includeGeometry: boolean): JsonObject[] {
  return (data.features ?? []).map((feature: JsonObject) => {
    const item: JsonObject = { attributes: normalizeAttributes(feature.attributes ?? {}) };
    if (includeGeometry) item.geometry = feature.geometry;
    return item;
  });
}

function normalizeAttributes(attrs: JsonObject): JsonObject {
  const clean: JsonObject = {};
  for (const [key, value] of Object.entries(attrs)) {
    if (key.toLowerCase().endsWith(".shape")) continue;
    clean[key] = typeof value === "string" ? value.trim() : value;
  }
  return clean;
}

function indicatorSummary(item: FlatIndicator): JsonObject {
  const node = item.node;
  return {
    code: node.code,
    table: tableName(String(node.code ?? "")),
...
function availableYears(node: JsonObject): number[] {
...
function latestYear(node: JsonObject): number {
  const years = availableYears(node);
  return years.length ? years[years.length - 1] : 0;
}

function regionLevelAvailability(): JsonObject {
  return {
...
function findAttribute(node: JsonObject, code: string): JsonObject | undefined {
  return (node.attributes ?? []).find((attr: JsonObject) => String(attr.code ?? "").toLowerCase() === code.toLowerCase());
}

function firstAttributeCode(node: JsonObject): string {
  const attrs = node.attributes ?? [];
  for (const attr of attrs) {
    const code = String(attr.code ?? "");
    if (code && !code.toLowerCase().endsWith("v")) return code.toUpperCase();
  }
  return attrs.length ? String(attrs[0].code ?? "").toUpperCase() : "";
}

function attributeTitle(node: JsonObject, code: string): string {
  return String(findAttribute(node, code)?.title_short ?? "");
}

function attributeUnit(node: JsonObject, code: string): string {
  return String(findAttribute(node, code)?.unit ?? "");
}

function attributeMeta(node: JsonObject, code: string): string {
  return String(findAttribute(node, code)?.meta ?? "");
}

function metaSnippets(meta: string, grep: string, limit: number): JsonObject[] {
  ...
  for (const line of lines) {
    if (!needle || line.toLowerCase().includes(needle)) snippets.push({ text: truncate(line, 700) });
...
function sourcesForIndicator(node: JsonObject, field: string, year: number): JsonObject[] {
...
function tableName(code: string): string {
...
function regionLevelLabel(level: number): string {
