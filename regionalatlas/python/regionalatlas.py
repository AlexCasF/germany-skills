...
def main(argv):
    ...
    try:
        if argv[0] == "doctor":
            ...
            raise CLIError(2, "unknown_command", "unknown command; run regionalatlas --help")
...
def print_root_help():
    print("""regionalatlas -- Regionalatlas Deutschland research CLI
...
def run_indicator_get(argv):
    ...
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"regionalatlas fields --indicator {item['node'].get('code')}", f"regionalatlas sample --indicator {item['node'].get('code')} --field {field} --year {year} --region-level 1 --limit 5"]
    emit(payload)
...
def run_fields(argv):
    ...
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"regionalatlas explain-field --indicator {node.get('code')} --field {field}", f"regionalatlas sample --indicator {node.get('code')} --field {field} --year {latest_year(node)} --region-level 1 --limit 5"]
    emit(payload)
...
def run_sample(argv):
    ...
    payload["warnings"] = warnings
    payload["nextActions"] = [f"regionalatlas query-builder --indicator {node.get('code')} --field {field.upper()} --year {year} --region-level {region_level} --limit {limit}", f"regionalatlas explain-field --indicator {node.get('code')} --field {field.upper()}"]
...
def run_source(argv):
    ...
    payload["sources"] = sources_for_indicator(node, field, year)
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"regionalatlas dossier --indicator {node.get('code')} --field {field.upper()} --year {year}"]
    emit(payload)
...
def run_dossier(argv):
    ...
    payload["sources"] = sources_for_indicator(node, field, year)
    payload["warnings"] = warnings
    payload["nextActions"] = [f"regionalatlas explain-field --indicator {node.get('code')} --field {field.upper()} --grep Quelle", f"regionalatlas query-builder --indicator {node.get('code')} --field {field.upper()} --year {year} --region-level {region_level}"]
    emit(payload)
...
def run_query_builder(argv):
    ...
    payload["sources"] = sources_for_indicator(item["node"], field, year)
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"regionalatlas sample --indicator {item['node'].get('code')} --field {field.upper()} --year {year} --region-level {region_level} --limit {limit}"]
    emit(payload)
...
def run_explain_field(argv):
    ...
    payload["warnings"] = default_warnings()
    payload["nextActions"] = [f"regionalatlas sample --indicator {node.get('code')} --field {attr.get('code', '').upper()} --year {latest_year(node)} --region-level 1 --limit 5"]
    emit(payload)
...
def next_actions_for_indicators(items):
    ...
    for item in items[:3]:
        node = item["node"]
        actions.append(f"regionalatlas dossier --indicator {node.get('code')} --field {first_attribute_code(node)} --year {latest_year(node)} --region-level 1")
    return actions or ['regionalatlas indicators search --term "Bevoelkerung" --limit 5']
