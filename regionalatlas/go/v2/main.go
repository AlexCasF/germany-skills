...
func main() {
	...
	default:
		err = cliError{2, "unknown_command", "unknown command; run regionalatlasctl --help"}
	}
	if err != nil {
		var ce cliError
		if errors.As(err, &ce) {
...
func printRootHelp() {
	fmt.Println(`regionalatlasctl -- Regionalatlas Deutschland research CLI
...
Do not use this when
  ...
  - you need Deutschlandatlas indicator tables; use deutschlandatlasctl
...
Fast paths
  ...
  Inspect metadata before data:
    ...
    regionalatlasctl explain-field --indicator AI008-1-5 --field AI0801
...
func printHelp(path []string) {
	...
	case "sample":
		fmt.Println(`regionalatlasctl sample

Fetch a small bounded sample from a Regionalatlas dynamic-layer query.

Examples
  regionalatlasctl sample --indicator AI008-1-5 --field AI0801 --year 2024 --region-level 1 --limit 5
  regionalatlasctl sample --indicator AI002-1-5 --field AI0201 --year 2020 --region-level 1 --limit 5
  regionalatlasctl sample --indicator AI008-1-5 --field AI0801 --year 2024 --ags 11
...
Safety
  Geometry is off by default. Limits above 100 require --allow-large-output.`)
	case "dossier":
		fmt.Println(`regionalatlasctl dossier

Build a compact evidence bundle: catalog metadata, fields, source URLs, query
URL, a tiny sample, warnings, and next actions.

Example
  regionalatlasctl dossier --indicator AI008-1-5 --field AI0801 --year 2024 --region-level 1`)
	case "query-builder":
		fmt.Println(`regionalatlasctl query-builder

Build the encoded ArcGIS dynamic-layer query URL without fetching data.

Example
  regionalatlasctl query-builder --indicator AI008-1-5 --field AI0801 --year 2024 --region-level 1`)
	case "query":
		fmt.Println(`regionalatlasctl query
...
func runIndicatorGet(argv []string) error {
	...
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{
		fmt.Sprintf("regionalatlasctl fields --indicator %s", item.Node.Code),
		fmt.Sprintf("regionalatlasctl sample --indicator %s --field %s --year %d --region-level 1 --limit 5", item.Node.Code, firstAttributeCode(item.Node), latestYear(item.Node)),
	}
	emit(payload)
	return nil
}
...
func runFields(argv []string) error {
	...
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{
		fmt.Sprintf("regionalatlasctl explain-field --indicator %s --field %s", item.Node.Code, firstAttributeCode(item.Node)),
		fmt.Sprintf("regionalatlasctl sample --indicator %s --field %s --year %d --region-level 1 --limit 5", item.Node.Code, firstAttributeCode(item.Node), latestYear(item.Node)),
	}
	emit(payload)
	return nil
...
func runSample(argv []string) error {
	...
	payload["warnings"] = warnings
	payload["nextActions"] = []string{
		fmt.Sprintf("regionalatlasctl query-builder --indicator %s --field %s --year %d --region-level %d --limit %d", item.Node.Code, strings.ToUpper(field), year, regionLevel, limit),
		fmt.Sprintf("regionalatlasctl explain-field --indicator %s --field %s", item.Node.Code, strings.ToUpper(field)),
	}
...
func runSource(argv []string) error {
	...
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{
		fmt.Sprintf("regionalatlasctl dossier --indicator %s --field %s --year %d", item.Node.Code, strings.ToUpper(field), year),
	}
	emit(payload)
	return nil
}
...
func runDossier(argv []string) error {
	...
	payload["warnings"] = warnings
	payload["nextActions"] = []string{
		fmt.Sprintf("regionalatlasctl explain-field --indicator %s --field %s --grep Quelle", item.Node.Code, strings.ToUpper(field)),
		fmt.Sprintf("regionalatlasctl query-builder --indicator %s --field %s --year %d --region-level %d", item.Node.Code, strings.ToUpper(field), year, regionLevel),
	}
	emit(payload)
	return nil
}
...
func runQueryBuilder(argv []string) error {
	...
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{
		fmt.Sprintf("regionalatlasctl sample --indicator %s --field %s --year %d --region-level %d --limit %d", item.Node.Code, strings.ToUpper(field), year, regionLevel, limit),
	}
	emit(payload)
	return nil
}
...
func runExplainField(argv []string) error {
	...
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{
		fmt.Sprintf("regionalatlasctl sample --indicator %s --field %s --year %d --region-level 1 --limit 5", item.Node.Code, strings.ToUpper(attr.Code), latestYear(item.Node)),
	}
	emit(payload)
	return nil
}
...
func fetchBytes(requestURL string) ([]byte, error) {
	...
	req.Header.Set("User-Agent", "democracy-researcher/regionalatlasctl-2.0")
...
func compactIndicators(items []flatIndicator, limit int) []any {
	...
	for i, item := range items {
		...
		out = append(out, map[string]any{
			...
			"nextActions": []string{
				fmt.Sprintf("regionalatlasctl fields --indicator %s", item.Node.Code),
				fmt.Sprintf("regionalatlasctl sample --indicator %s --field %s --year %d --region-level 1 --limit 5", item.Node.Code, firstField, latestYear(item.Node)),
			},
		})
	}
	return out
...
func nextActionsForIndicators(items []flatIndicator) []string {
	...
	for i, item := range items {
		...
		actions = append(actions, fmt.Sprintf("regionalatlasctl dossier --indicator %s --field %s --year %d --region-level 1", item.Node.Code, firstAttributeCode(item.Node), latestYear(item.Node)))
	}
	if len(actions) == 0 {
		return []string{`regionalatlasctl indicators search --term "Bevoelkerung" --limit 5`}
	}
	return actions
}
