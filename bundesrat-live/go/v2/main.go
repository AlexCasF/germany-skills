...
func main() {
	...
	default:
		err = cliError{2, "unknown_command", "unknown command; run bundesratctl --help"}
	}
...
func printRootHelp() {
	fmt.Println(`bundesratctl -- Bundesrat live/app XML research CLI
...
Do not use this when
  - you need the full parliamentary archive or Bundestag proceedings; use dipctl
  - you need federal law text; use rechtsinformationenctl
...
Fast paths
  ...
  bundesratctl page --url "https://www.bundesrat.de/SharedDocs/pm/2026/10_europaeischer_verdienstorden_merkel.html" --grep "Merkel"
...
func printHelp(path []string) {
	...
	case "news search", "dates search":
		fmt.Println(`bundesratctl news search

Search a Bundesrat feed and return compact source-rich rows.

Examples
  bundesratctl news search --term "Bovenschulte" --limit 3
  bundesratctl dates search --term "Ausschuss" --limit 5
...
Flags
  ...
  --include-raw       Include raw XML block for matching records`)
	case "members search":
		fmt.Println(`bundesratctl members search
...
Flags
  ...
  --include-raw       Include raw XML block for matching records`)
	case "members dossier":
		fmt.Println(`bundesratctl members dossier
...
Flags
  ...
  --include-raw       Include raw XML block`)
	case "plenum compact", "plenum current", "plenum dossier":
		fmt.Println(`bundesratctl plenum compact

Normalize Bundesrat plenary summary or agenda TOPs.

Examples
  bundesratctl plenum compact --limit 1 --top-limit 3
  bundesratctl plenum current --limit 1 --top-limit 5 --grep "Drucksache"
  bundesratctl plenum dossier --grep "Sozialleistungsbetrug" --top-limit 5
...
4. Search news and expand a returned source page:
   bundesratctl news search --term "Bovenschulte" --limit 3
   bundesratctl news page --url "https://www.bundesrat.de/SharedDocs/pm/2026/10_europaeischer_verdienstorden_merkel.html" --grep "Bovenschulte"
...
10. Fetch raw XML if the normalized summary is insufficient:
   bundesratctl plenum compact --raw`)
}
...
func runMemberDossier(argv []string) error {
	...
	if sourceURL := fmt.Sprint(item["url"]); sourceURL != "" {
		...
		payload["nextActions"] = []string{fmt.Sprintf("bundesratctl page --url %q --grep %q", sourceURL, firstNonEmpty(grep, "Bundesrat"))}
	}
	payload["warnings"] = defaultWarnings()
	emit(payload)
	return nil
...
func runPlenumNext(argv []string) error {
	...
	payload["warnings"] = defaultWarnings()
	payload["nextActions"] = []string{"bundesratctl plenum current --limit 1 --top-limit 5", "bundesratctl plenum compact --limit 1 --top-limit 5"}
	emit(payload)
	return nil
}
...
func runPage(command string, argv []string) error {
	...
	payload["nextActions"] = []string{fmt.Sprintf("bundesratctl source --url %q", sourceURL)}
	emit(payload)
	return nil
}
...
func runSource(argv []string) error {
	...
	payload["warnings"] = defaultWarnings()
	if strings.HasPrefix(sourceURL, baseURL+"/") {
		payload["nextActions"] = []string{fmt.Sprintf("bundesratctl page --url %q", sourceURL)}
	}
	emit(payload)
	return nil
}
...
func nextActionsFromItems(items []map[string]any, key string) []string {
	...
	if key == "news" {
		return []string{`bundesratctl news search --term "Bovenschulte" --limit 3`}
	}
	if key == "dates" {
		return []string{`bundesratctl dates search --term "Ausschuss" --limit 5`}
	}
	return []string{"bundesratctl plenum compact --limit 1 --top-limit 3"}
}
...
func nextActionsFromEmployees(items []map[string]any) []string {
	...
	for _, item := range items {
		if name := fmt.Sprint(item["name"]); name != "" {
			actions = append(actions, fmt.Sprintf("bundesratctl members dossier --name %q", name))
		}
		if len(actions) >= 3 {
			break
		}
...
func nextActionsForURL(sourceURL string, key string) []string {
	...
	var actions []string
	if strings.HasPrefix(sourceURL, baseURL+"/") {
		actions = append(actions, fmt.Sprintf("bundesratctl page --url %q", sourceURL))
	}
	if key == "news" {
		actions = append(actions, fmt.Sprintf("bundesratctl news page --url %q", sourceURL))
	}
	if key == "dates" {
		actions = append(actions, fmt.Sprintf("bundesratctl dates page --url %q", sourceURL))
	}
	return actions
}
