...
async function main(argv: string[]): Promise<number> {
  ...
  try {
    ...
    else if (argv[0] === "startlist") await runFeed("startlist", argv.slice(1));
    else if (argv[0] === "news" && argv[1] === "search") await runFeedSearch("news", argv.slice(2));
    else if (argv[0] === "news" && argv[1] === "page") await runPage("news page", argv.slice(2));
    else if (argv[0] === "news") await runFeed("news", argv.slice(1));
    else if (argv[0] === "dates" && argv[1] === "search") await runFeedSearch("dates", argv.slice(2));
    else if (argv[0] === "dates" && argv[1] === "page") await runPage("dates page", argv.slice(2));
    else if (argv[0] === "dates") await runFeed("dates", argv.slice(1));
    else if (matches(argv, "plenum", "compact")) await runPlenum("plenum compact", argv.slice(2));
    else if (matches(argv, "plenum", "current")) await runPlenum("plenum current", argv.slice(2));
    else if (matches(argv, "plenum", "chronological")) await runPlenum("plenum chronological", argv.slice(2));
    else if (matches(argv, "plenum", "next")) await runPlenumNext(argv.slice(2));
    else if (matches(argv, "plenum", "dossier")) await runPlenum("plenum compact", argv.slice(2));
    else if (argv[0] === "members" && argv[1] === "search") await runMembersSearch(argv.slice(2));
    else if (argv[0] === "members" && argv[1] === "dossier") await runMemberDossier(argv.slice(2));
    else if (argv[0] === "members") await runMembers(argv.slice(1));
    else if (argv[0] === "votes" && argv[1] === "summary") await runFeed("votes", argv.slice(2));
    else if (argv[0] === "votes") await runFeed("votes", argv.slice(1));
...
async function runFeed(key: string, argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
...
async function runFeedSearch(key: string, argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const term = firstNonEmpty(parsed.flags.term, parsed.flags.q, parsed.flags.name, parsed.positionals.join(" "));
  if (!term) throw new CLIError(2, "missing_term", `${key} search requires --term`);
  parsed.flags.grep = term;
  await runFeed(key, rebuildArgs(parsed));
}

async function runMembers(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
...
async function runMemberDossier(argv: string[]): Promise<void> {
  ...
  if (!term) throw new CLIError(2, "missing_member", "members dossier requires --name or --url");
  const { body, requestUrl } = await fetchEndpoint("members", {});
  const grep = parsed.flags.grep ?? "";
  const matchesFound = compactEmployees(body, SAFE_LIMIT, term, flagBool(parsed, "include-raw"));
  if (!matchesFound.length) throw new CLIError(2, "member_not_found", `member not found in current Bundesrat feed: ${term}`);
...
async function runPlenum(key: string, argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const { body, requestUrl } = await fetchEndpoint(key, parsed.params);
  if (flagBool(parsed, "raw")) return printRaw(body);
  const limit = limitFlag(parsed, DEFAULT_LIMIT, SAFE_LIMIT);
...
async function runPlenumNext(argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
...
async function runPage(command: string, argv: string[]): Promise<void> {
  const parsed = parseArgs(argv);
  const sourceUrl = firstNonEmpty(parsed.flags.url, parsed.flags["source-url"], parsed.positionals.join(" "));
  if (!sourceUrl) throw new CLIError(2, "missing_url", `${command} requires --url`);
  if (!sourceUrl.startsWith(`${BASE_URL}/`)) throw new CLIError(2, "unsafe_url", "page only accepts https://www.bundesrat.de URLs");
...
async function fetchEndpoint(key: string, params: Record<string, string>): Promise<{ body: string; requestUrl: string }> {
  if (!ENDPOINTS[key]) throw new CLIError(2, "unknown_endpoint", `unknown endpoint: ${key}`);
...
function withDefaultView(base: string, params: Record<string, string>): string {
  const query = new URLSearchParams({ ...(params ?? {}) });
  if (!query.has("view")) query.set("view", "renderXml");
  return `${base}?${query.toString()}`;
}

function compactItems(raw: string, key: string, limit: number, grep: string, includeRaw: boolean): JsonObject[] {
  const out: JsonObject[] = [];
  for (const item of blocks(raw, "item")) {
    if (grep && !itemSearchText(item).toLowerCase().includes(grep.toLowerCase())) continue;
...
function compactItem(item: string, key: string, grep: string, includeRaw: boolean): JsonObject {
  ...
  if (includeRaw) out.raw = item;
  return out;
}

function compactEmployees(raw: string, limit: number, term: string, includeRaw: boolean): JsonObject[] {
  const out: JsonObject[] = [];
  for (const employee of blocks(raw, "employee")) {
    if (term && !employeeSearchText(employee).toLowerCase().includes(term.toLowerCase())) continue;
    ...
    if (includeRaw) item.raw = employee;
    out.push(item);
    if (out.length >= limit) break;
  }
  return out;
...
function compactPlenum(raw: string, key: string, limit: number, topLimit: number, grep: string, includeRaw: boolean): JsonObject[] {
  const out: JsonObject[] = [];
...
function itemSearchText(item: string): string {
...
function employeeSearchText(employee: string): string {
...
function nextActionsFromItems(items: JsonObject[], key: string): string[] {
...
function rebuildArgs(parsed: ParsedArgs): string[] {
  const args: string[] = [];
  for (const [key, value] of Object.entries(parsed.flags)) args.push(`--${key}`, value);
  for (const [key, value] of Object.entries(parsed.params)) args.push("--param", `${key}=${value}`);
...
function countItemLike(raw: string): number {
...
function sourceKind(sourceUrl: string): string {
  ...
  if (sourceUrl.includes("dip.bundestag.de")) return "dip_reference";
  if (sourceUrl.includes("/SharedDocs/personen/")) return "public_profile";
  if (sourceUrl.includes("/SharedDocs/drucksachen/") || sourceUrl.includes("/drs.html")) return "official_document";
  if (sourceUrl.includes("/DE/plenum/")) return "plenary_page";
  if (sourceUrl.includes("/SharedDocs/pm/")) return "press_release";
  return "public_page";
}
...
function extractLinks(value: string, limit: number): JsonObject[] {
  const out: JsonObject[] = [];
  const seen = new Set<string>();
  for (const match of String(value || "").matchAll(/<a\s+[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
    let rawUrl = decodeEntities(match[1]).trim();
    if (!rawUrl || rawUrl.startsWith("mailto:") || rawUrl.startsWith("tel:")) continue;
    if (rawUrl.startsWith("/")) rawUrl = BASE_URL + rawUrl;
    if (!rawUrl.startsWith("http")) rawUrl = `${BASE_URL}/${rawUrl.replace(/^\.\//, "")}`;
...
function tableRows(value: string): JsonObject[] {
