...
func printRootHelp() {
	fmt.Println(`lobbyregisterctl -- Bundestag Lobbyregister research CLI
...
func printHelp(path []string) {
	...
	switch joined {
	case "entry dossier":
		fmt.Println(`lobbyregisterctl entry dossier
...
Examples
  ...
  lobbyregisterctl entry dossier --name "Bundesverband Soziokultur"`)
	case "search":
		fmt.Println(`lobbyregisterctl search
...
Examples
  ...
  lobbyregisterctl search --term "\"Bundesverband Soziokultur\"" --include-raw`)
	case "entry get":
		fmt.Println(`lobbyregisterctl entry get

Fetch one official V2 register entry by register number.

Examples
  lobbyregisterctl entry get --register-number R001255
  lobbyregisterctl entry get --register-number R001255 --version 6 --include-raw`)
	case "financial summary":
		fmt.Println(`lobbyregisterctl financial summary
...
func runDoctor(argv []string) error {
	...
	if key == "" {
		...
		payload["nextActions"] = []string{"Set LOBBYREGISTER_API_KEY, then run: lobbyregisterctl statistics"}
		emit(payload)
		return nil
	}
...
func runStatistics(argv []string) error {
	...
	payload["sources"] = defaultSources()
	payload["nextActions"] = []string{`lobbyregisterctl search --term "Energie" --limit 5`}
	emit(payload)
	return nil
}
...
func searchNextActions(items []any) []string {
	...
	for _, item := range items {
		...
		if rn := stringAt(m, "registerNumber"); rn != "" {
			out = append(out, "lobbyregisterctl entry dossier --register-number "+rn)
		}
		if len(out) >= 5 {
			break
		}
