...
def print_root_help():
    print("""lobbyregisterctl -- Bundestag Lobbyregister research CLI
...
def print_help(path):
    ...
    if joined == "entry dossier":
        print("""lobbyregisterctl entry dossier
...
def next_actions_for_entry(entry):
    ...
    if not rn:
        return []
    return [
        f"lobbyregisterctl entry source --register-number {rn}",
        f"lobbyregisterctl financial summary --register-number {rn}",
        f"lobbyregisterctl statements list --register-number {rn} --grep <term>",
    ]


def search_next_actions(items):
    return [f"lobbyregisterctl entry dossier --register-number {item['registerNumber']}" for item in items if item.get("registerNumber")][:5]
